...
# ------------------------------------------------------------------
# AFI_UPLOAD_DEAS_VERSICHERUNG_PDF_V0482
# Datum: 2026-07-09
# Zweck:
# - Lieferant DEAS im AFI-Upload unterstuetzen.
# - DEAS-Prämienrechnungen werden je Fahrzeugzeile aus der Spalte "Aufforderung" gelesen.
# - Versicherungssteuer ist nicht vorsteuerabzugsfaehig: Brutto = Netto, TAX_CODE = V0.
# - Kostenbeschreibung wird fuer DEAS fachlich auf "Versicherung" gesetzt.
# - KST-/Sachkonto-Zuordnung erfolgt kennzeichenbasiert inkl. E-Kennzeichen-Fallback.
# - Weiterberechnungs-KST greifen ueber die bestehende Intercompany-Logik.
# ------------------------------------------------------------------
AFI_UPLOAD_DEAS_VERSICHERUNG_PDF_VERSION = "0.482"
_DEAS_LAST_PARSED_POSITIONS = []
_DEAS_PLATE_RE = re.compile(r"\b[A-ZÄÖÜ]{1,3}\s*-\s*[A-ZÄÖÜ]{1,3}\s*\d{1,5}[A-ZÄÖÜ]?\b", re.I)


def _deas_clean_plate(value):
    value = _clean(value).upper()
    value = re.sub(r"\s*-\s*", "-", value)
    value = re.sub(r"\s+", " ", value)
    return value
...
def _deas_plate_e_variant(value):
    plate = _deas_clean_plate(value)
    if not plate:
        return plate
    if re.search(r"E$", plate, flags=re.I):
        return plate
    return plate + "E"


def _deas_is_invoice_text(text):
    n = _norm(text)
    return ("DEAS" in n or "DEUTSCHEASSEKURANZMAKLER" in n) and "PRAEMIENRECHNUNG" in n and "AUFFORDERUNG" in n


def _deas_is_invoice_path(path):
    try:
        if "DEAS" in _norm(os.path.basename(path or "")):
            return True
        return _deas_is_invoice_text(_extract_pdf_text(path))
    except Exception:
        return False


def _deas_amount_from_piece(piece):
    # Betrag der Spalte Aufforderung steht unmittelbar hinter den beiden Abrechnungsdaten.
    m = re.search(r"\b\d{2}\.\d{2}\.\d{4}\s+\d{2}\.\d{2}\.\d{4}\s+(-?\d{1,3}(?:\.\d{3})*,\d{2})\s*€", piece)
    if not m:
        return Decimal("0.00")
    return _dec(m.group(1))


def _parse_deas_versicherung_pdf_positions(path, global_prefix=None):
    text = _extract_pdf_text(path)
    if not _deas_is_invoice_text(text):
        raise RuntimeError("Keine DEAS-Versicherungsrechnung erkannt.")
    compact = " ".join(text.replace("\n", " ").split())
    row_re = re.compile(
        r"\b(?P<nr>\d{1,3})\s+(?P<plate>[A-ZÄÖÜ]{1,3}\s*-\s*[A-ZÄÖÜ]{1,3}\s*\d{1,5}[A-ZÄÖÜ]?)\s+"
        r"(?P<body>.*?\b\d{2}\.\d{2}\.\d{4}\s+\d{2}\.\d{2}\.\d{4}\s+-?\d{1,3}(?:\.\d{3})*,\d{2}\s*€)"
        r"(?P<tail>.*?)(?=\s+\d{1,3}\s+[A-ZÄÖÜ]{1,3}\s*-\s*[A-ZÄÖÜ]{1,3}\s*\d|\s+zu zahlender Betrag|\s+Seite\s+\d|$)",
        re.I,
    )
    positions = []
    seen_rows = set()
    for m in row_re.finditer(compact):
        try:
            nr = int(m.group('nr'))
        except Exception:
            nr = 0
        if not nr or nr in seen_rows:
            continue
        amount = _deas_amount_from_piece(m.group('body'))
        if amount == 0:
            continue
        plate = _deas_clean_plate(m.group('plate'))
        tail = m.group('tail') or ''
        # In der PDF steht "E-Kennzeichen" als Risikoinformation nach der Zeile. Fuer die Kontierungsdatei
        # wird zusaetzlich ohnehin ein E-Fallback versucht; hier wird der sichtbare Schluessel bereits passend gesetzt.
        if re.search(r"\bE\s*-?\s*Kennzeichen\b", tail, flags=re.I):
            plate = _deas_plate_e_variant(plate)
        positions.append({
            'row_no': nr,
            'key': plate,
            'driver': plate,
            ...
            'tax': 'V0',
            'foreign_gross': False,
            'source_label': 'Versicherung',
            'deas_gross_equals_net': True,
        })
        seen_rows.add(nr)
    if not positions:
        raise RuntimeError("Aus der DEAS-PDF konnten keine Fahrzeugpositionen mit Aufforderungsbetrag erkannt werden.")
    positions.sort(key=lambda p: p.get('row_no', 0))
    global _DEAS_LAST_PARSED_POSITIONS
    _DEAS_LAST_PARSED_POSITIONS = positions
    return positions


def _deas_assignment_by_plate(entries):
    out = {}
    for e in entries or []:
        ident = _deas_clean_plate(e.get('identifier', ''))
        if ident:
            out[_norm(ident)] = e
            if ident.endswith('E'):
                out.setdefault(_norm(ident[:-1]), e)
            else:
                out.setdefault(_norm(ident + 'E'), e)
    return out


def _deas_resolve_entry(by_plate, plate):
    plate = _deas_clean_plate(plate)
    for candidate in (plate, _deas_plate_e_variant(plate), plate[:-1] if plate.endswith('E') else ''):
        if candidate and _norm(candidate) in by_plate:
            return by_plate[_norm(candidate)]
    return None


def _deas_display_name(entry, fallback):
    if not entry:
        return fallback
    first = _clean(entry.get('first', ''))
    last = _clean(entry.get('last', ''))
    full = _clean(entry.get('full_name', ''))
    return _clean(f"{first} {last}") or full or fallback


def _deas_create_export(assignment_path, invoice_path, export_path, config):
    cfg = dict(config or {})
    cfg['global_prefix'] = 'Versicherung'
    cfg['supplier'] = 'DEAS'
    if '_afi478_update_context' in globals():
        _afi478_update_context(assignment_path, cfg)
    entries = load_assignment_entries(assignment_path)
    by_plate = _deas_assignment_by_plate(entries)
    positions = _parse_deas_versicherung_pdf_positions(invoice_path, 'Versicherung')
    groups = OrderedDict()
    warnings_missing = []
    unique_keys = set()
    total = Decimal('0.00')
    for pos in positions:
        plate = _deas_clean_plate(pos.get('key', ''))
        entry = _deas_resolve_entry(by_plate, plate)
        display = _deas_display_name(entry, plate)
        if entry:
            gl, cc, orderid = _select_assignment_values(entry, 'VERSICHERUNG', 'Versicherung')
        else:
            gl, cc, orderid = '', '', ''
            warnings_missing.append(f"{plate}: keine eindeutige Kennzeichen-Zuordnung")
        # Falls Sachkonto fehlt, generaluebersicht-GL DEAS als Fallback je Buchungskreis verwenden.
        if not gl:
            try:
                bukrs = _afi478_bukrs_label((entry or {}).get('bukrs') or (config or {}).get('booking_circle') or 'IDE') if '_afi478_bukrs_label' in globals() else 'IDE'
                fb = _afi471_fallback_entry_for_gl(assignment_path, 'Versicherung', bukrs) if '_afi471_fallback_entry_for_gl' in globals() else {}
                gl = (fb or {}).get('gl_versicherung') or (fb or {}).get('gl_default') or '419500'
            except Exception:
                gl = '419500'
        amount = Decimal(pos.get('amount', '0'))
        total += amount
        unique_keys.add(_norm(plate))
        text = _clean(f"Versicherung {plate} {display}" if _norm(display) != _norm(plate) else f"Versicherung {plate}")
        gkey = (_norm(text), 'V0', gl or '', cc or '', orderid or '')
        if gkey not in groups:
            groups[gkey] = {'TEXT': text, 'amount': Decimal('0.00'), 'TAX_CODE': 'V0', 'GL_ACCOUNT': gl or '', 'COSTCENTER': cc or '', 'ORDERID': orderid or '', 'driver_norm': _norm(display)}
        groups[gkey]['amount'] += amount
    ordered = sorted(groups.values(), key=lambda g: (g.get('driver_norm',''), g.get('TEXT','')))
    os.makedirs(os.path.dirname(os.path.abspath(export_path)) or '.', exist_ok=True)
    ...
    with open(export_path, 'w', encoding='utf-8-sig', newline='') as f:
        ...
        for g in ordered:
            amount = Decimal(g.get('amount', '0')).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
            export_total += amount
            writer.writerow({'TEXT': g.get('TEXT',''), 'PRICE': _fmt(amount), 'PRICE_UNIT':'1', 'QUANTITY':'1', 'UNIT':'ST', 'NET_VALUE': _fmt(amount), 'TAX_CODE':'V0', 'GL_ACCOUNT': g.get('GL_ACCOUNT',''), 'COSTCENTER': g.get('COSTCENTER',''), 'ORDERID': g.get('ORDERID','')})
    return {
        'rows': len(ordered),
        'export_path': export_path,
        'invoice_net_raw_total': _fmt(total),
        'export_net_total': _fmt(export_total),
        'net_rounding_difference': _fmt(export_total - total.quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)),
        'unique_drivers': 0,
        'unique_keys': len([x for x in unique_keys if x]),
        'missing_template': warnings_missing,
        'unknown_tax': [],
        ...
        'foreign_gross': [],
        'deas_positions': len(positions),
        'deas_gross_equals_net': True,
        'deas_tax_code': 'V0',
    }


_parse_pdf_invoice_positions_before_v0482_deas = _parse_pdf_invoice_positions

def _parse_pdf_invoice_positions(path, global_prefix):
    try:
        if _deas_is_invoice_path(path):
            return _parse_deas_versicherung_pdf_positions(path, 'Versicherung')
    except Exception as exc:
        try:
            if 'DEAS' in _norm(os.path.basename(path or '')):
                raise exc
            ...
            raise
    return _parse_pdf_invoice_positions_before_v0482_deas(path, global_prefix)


_create_supplier_upload_csv_before_v0482_deas = create_supplier_upload_csv

def create_supplier_upload_csv(assignment_path, invoice_path, export_path, config):
    cfg = dict(config or {})
    paths = _afi480_split_invoice_paths(invoice_path) if '_afi480_split_invoice_paths' in globals() else ([invoice_path] if invoice_path else [])
    try:
        if len(paths) == 1 and os.path.splitext(paths[0])[1].lower() == '.pdf' and _deas_is_invoice_path(paths[0]):
            result = _deas_create_export(assignment_path, paths[0], export_path, cfg)
            try:
                if '_afi478_sanitize_export_file' in globals():
                    changed = _afi478_sanitize_export_file(export_path)
                    if isinstance(result, dict): result['text_sanitized_long_digits'] = changed
            finally:
                if '_AFI478_CONTEXT' in globals():
                    _AFI478_CONTEXT.update({'booking_circle':'IDE','intercompany':True,'assignment_path':''})
            return result
        return _create_supplier_upload_csv_before_v0482_deas(assignment_path, invoice_path, export_path, config)
    finally:
        if '_AFI478_CONTEXT' in globals():
            _AFI478_CONTEXT.update({'booking_circle':'IDE','intercompany':True,'assignment_path':''})


_fm_short_vendor_before_v0482_deas = _fm_short_vendor if '_fm_short_vendor' in globals() else None

def _fm_short_vendor(path):
    n = _norm(os.path.basename(path or ''))
    if 'DEAS' in n or 'DEUTSCHEASSEKURANZ' in n:
        return 'DEAS'
    return _fm_short_vendor_before_v0482_deas(path) if _fm_short_vendor_before_v0482_deas else (os.path.splitext(os.path.basename(path or 'Rechnung'))[0] or 'Rechnung')


# DEAS in den sichtbaren Optionen fachlich absichern.
try:
    if 'DEAS' not in SUPPLIER_OPTIONS:
        SUPPLIER_OPTIONS = list(SUPPLIER_OPTIONS) + ['DEAS']
    if 'Versicherung' not in COST_TYPE_OPTIONS:
        COST_TYPE_OPTIONS = list(COST_TYPE_OPTIONS) + ['Versicherung']
except Exception:
    pass

# ------------------------------------------------------------------
# AFI_UPLOAD_UI_FONT_EXPORT_TITLE_V0483
# Datum: 2026-07-09
# Zweck:
# - Text in Dropdown-Menues und Buttons besser lesbar vergroessern.
# - Button-Schriften werden adaptiv gesetzt, damit lange Beschriftungen nicht aus Buttons laufen.
# - Exporttitel-/Dateinamenlogik: Buchungskreis_Kostenbeschreibung_JJJJ_MM_TT.csv
#   Die ausgewaehlte Kostenbeschreibung aktualisiert den Exportpfad dynamisch.
# ------------------------------------------------------------------
AFI_UPLOAD_UI_FONT_EXPORT_TITLE_VERSION = "0.483"
AFI483_UI_FONT = ("Segoe UI", 10)
AFI483_DROPDOWN_FONT = ("Segoe UI", 10)
AFI483_BUTTON_FONT = ("Segoe UI", 10)
AFI483_BUTTON_FONT_LONG = ("Segoe UI", 9)
AFI483_BUTTON_FONT_BOLD = ("Segoe UI", 10, "bold")


def _afi483_safe_name(value):
    try:
        return _fm_safe_name_part(value)
    except Exception:
        s = _clean(value) if '_clean' in globals() else str(value or '').strip()
        s = re.sub(r"[^A-Za-z0-9ÄÖÜäöüß]+", "_", s, flags=re.I).strip("_")
        return s or "Kosten"


def _afi483_button_font_for_text(text, explicit_bold=False):
    text = str(text or '')
    if explicit_bold:
        return AFI483_BUTTON_FONT_BOLD
    # Lange Texte bleiben minimal kleiner, damit nichts aus dem Button laeuft.
    return AFI483_BUTTON_FONT_LONG if len(text) > 24 else AFI483_BUTTON_FONT


def _afi483_configure_button_readability(btn):
    try:
        txt = str(btn.cget('text') or '')
        current_font = btn.cget('font')
        bold = 'bold' in str(current_font).lower() or 'AFI-Upload' in txt
        btn.configure(font=_afi483_button_font_for_text(txt, explicit_bold=bold))
        # Kleine Innenabstaende verbessern Lesbarkeit, ohne die Layoutbreite hart zu vergroessern.
        try: btn.configure(padx=max(6, int(btn.cget('padx') or 0)))
        except Exception: pass
        try: btn.configure(pady=max(3, int(btn.cget('pady') or 0)))
        except Exception: pass
        if len(txt) > 28:
            try: btn.configure(wraplength=170, justify='center')
            except Exception: pass
    except Exception:
        pass


def _afi483_configure_combobox_readability(cb):
    try:
        cb.configure(font=AFI483_DROPDOWN_FONT)
    except Exception:
        pass
    try:
        root = cb.winfo_toplevel()
        root.option_add('*TCombobox*Listbox.font', AFI483_DROPDOWN_FONT)
    except Exception:
        pass


# Global fuer dieses Tool: neu erzeugte Buttons/Dropdowns lesbarer machen.
try:
    if not hasattr(tk.Button, '_afi483_orig_init'):
        tk.Button._afi483_orig_init = tk.Button.__init__
        def _afi483_button_init(self, master=None, cnf={}, **kw):
            tk.Button._afi483_orig_init(self, master, cnf, **kw)
            _afi483_configure_button_readability(self)
        tk.Button.__init__ = _afi483_button_init
except Exception:
    pass

try:
    if not hasattr(ttk.Combobox, '_afi483_orig_init'):
        ttk.Combobox._afi483_orig_init = ttk.Combobox.__init__
        def _afi483_combobox_init(self, master=None, **kw):
            if 'font' not in kw:
                kw['font'] = AFI483_DROPDOWN_FONT
            ttk.Combobox._afi483_orig_init(self, master, **kw)
            _afi483_configure_combobox_readability(self)
        ttk.Combobox.__init__ = _afi483_combobox_init
except Exception:
    pass


def _afi483_selected_bukrs_for_filename(self):
    try:
        vals = _afi463_selected_bukrs(self)
        return '-'.join(vals or ['IDE'])
    except Exception:
        try:
            if hasattr(self, 'booking_circle_var'):
                return self.booking_circle_var.get() or 'IDE'
...
def _afi483_selected_cost_for_filename(self):
    try:
        val = self.global_prefix_var.get() if hasattr(self, 'global_prefix_var') else ''
    except Exception:
        val = ''
    return _clean(val) if '_clean' in globals() else str(val or '').strip() or 'Kosten'


def _afi483_update_export_path(self, force=False):
    if not hasattr(self, 'export_var'):
        return
    current = self.export_var.get().strip() if self.export_var.get() else ''
    bukrs = _afi483_selected_bukrs_for_filename(self)
    cost = _afi483_selected_cost_for_filename(self) or 'Kosten'
    try:
        base = AFI_EXPORT_DEFAULT_DIR
    except Exception:
        base = _desktop_path() if '_desktop_path' in globals() else os.getcwd()
    today = _fm_dt.datetime.now().strftime('%Y_%m_%d') if '_fm_dt' in globals() else datetime.now().strftime('%Y_%m_%d')
    filename = f"{_afi483_safe_name(bukrs)}_{_afi483_safe_name(cost)}_{today}.csv"
    new_path = os.path.join(base, filename)
    default_dirs = {'', os.path.abspath(base)}
    try:
        default_dirs.add(os.path.abspath(_desktop_path()))
    except Exception:
        pass
    cur_dir = os.path.abspath(os.path.dirname(current)) if current else ''
    if force or not current or cur_dir in default_dirs or current.startswith(base):
        self.export_var.set(new_path)


# Alle bisherigen Update-Helfer auf die neue Namenslogik umbiegen.
try:
    _afi463_update_export_path = _afi483_update_export_path
    _fm_update_export_path = _afi483_update_export_path
except Exception:
    pass


def _afi483_install_dynamic_export_traces(self):
    if getattr(self, '_afi483_export_trace_installed', False):
        return
    def _cb(*_):
        try:
            _afi483_update_export_path(self, True)
        except Exception:
            pass
    try:
        if hasattr(self, 'global_prefix_var'):
            self.global_prefix_var.trace_add('write', _cb)
        if hasattr(self, 'booking_circle_var'):
            self.booking_circle_var.trace_add('write', _cb)
        if hasattr(self, 'booking_circle_vars'):
            for _b, var in self.booking_circle_vars.items():
                try: var.trace_add('write', _cb)
                except Exception: pass
    except Exception:
        pass
    self._afi483_export_trace_installed = True


def _afi483_walk_widgets(widget):
    try:
    ...
    for child in children:
        try:
            if isinstance(child, tk.Button):
                _afi483_configure_button_readability(child)
            elif isinstance(child, ttk.Combobox):
                _afi483_configure_combobox_readability(child)
        except Exception:
            pass
        _afi483_walk_widgets(child)


try:
    _afi483_build_left_before = SupplierUploadUI._build_left
    def _afi483_build_left(self, parent):
        # Vor dem Rendern setzen, damit Dropdowns und Buttons bereits lesbarer erstellt werden.
        try:
            self.font_small = AFI483_UI_FONT
        except Exception:
            pass
        result = _afi483_build_left_before(self, parent)
        try:
            _afi483_install_dynamic_export_traces(self)
            _afi483_update_export_path(self, True)
            _afi483_walk_widgets(parent)
        except Exception:
            pass
        return result
    SupplierUploadUI._build_left = _afi483_build_left
except Exception:
    pass

try:
    _afi483_init_before = SupplierUploadUI.__init__
    def _afi483_init(self, *args, **kwargs):
        result = _afi483_init_before(self, *args, **kwargs)
        try:
            self.font_small = AFI483_UI_FONT
        except Exception:
            pass
        return result
    SupplierUploadUI.__init__ = _afi483_init
except Exception:
    pass

# ------------------------------------------------------------------
# AFI_UPLOAD_KAZENMAIER_BIKE_LEASING_PDF_V0484
# Datum: 2026-07-09
# Zweck:
# - Lieferant Kazenmaier fuer Bike-Leasing integrieren.
# - Namen aus Kazenmaier-Rechnung werden als fuehrende Zuordnung verwendet; es gibt keine Kennzeichen.
# - KST wird aus dem Blatt "Bike" der Kontierungszuordnung_Generaluebersicht anhand des Namens ermittelt.
# - Sachkonto wird ueber Sachkonten / "Bike Leasing" ermittelt.
# - Intercompany-/Weiterberechnungs-KST greifen fuer Kazenmaier nicht; es wird immer die persoenliche KST verwendet.
# ------------------------------------------------------------------
AFI_UPLOAD_KAZENMAIER_BIKE_LEASING_PDF_VERSION = "0.484"
_KAZENMAIER_LAST_PARSED_POSITIONS = []
_KAZENMAIER_BIKE_CACHE = {"path":"", "mtime":None, "items":[], "gl":{}}


def _kaz_is_invoice_text(text):
    n = _norm(text)
    return ("KAZENMAIER" in n and "EBIKELEASING" in n and "LEASINGVERTRAEGE" in n)


def _kaz_is_invoice_path(path):
    try:
        if "KAZENMAIER" in _norm(os.path.basename(path or "")):
            return True
        return _kaz_is_invoice_text(_extract_pdf_text(path))
    except Exception:
        return False


def _kaz_invoice_bukrs_from_text(text):
    n = _norm(text)
    if "INTERSPORTMARKETINGSERVICES" in n:
        return "IMS"
    if "INTERSPORTDIGITAL" in n:
        return "IDG"
    if "SABU" in n:
        return "SABU"
    return "IDE"
...
def _kaz_extract_pdf_positions(path):
    text = _extract_pdf_text(path)
    if not _kaz_is_invoice_text(text):
        raise RuntimeError("Keine Kazenmaier-Bike-Leasing-PDF erkannt.")
    compact = " ".join(text.replace("\n", " ").split())
    money = r"\d{1,3}(?:\.\d{3})*,\d{2}"
    # Vertragsnummern koennen 5- oder 6-stellig sein. Zwischen Vertragskopf und Summe duerfen Seitenumbrueche liegen.
    pattern = re.compile(
        r"(?P<contract>\d{5,6})\*?(?P<body>.*?)(?:\*?\s*)Summe\s+(?P=contract)\s+"
        r"(?P<leasing>" + money + r")\s+(?P<service>" + money + r")\s+(?P<total>" + money + r")",
        re.I | re.S,
    )
    name_re = re.compile(r"/\s+([A-Za-zÄÖÜäöüßÀ-ÿ .'-]+?)\s+[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{4}\b", re.I)
    out = []
    for m in pattern.finditer(compact):
        body = m.group('body') or ''
        names = list(name_re.finditer(body))
        name = _clean(names[-1].group(1)) if names else ''
        if not name:
            continue
        amount = _dec(m.group('total'))
        if amount == 0:
            continue
        out.append({
            'contract': _clean(m.group('contract')),
            'driver': name,
            'amount': amount,
            'tax': 'VD',
            'source_label': 'Bike Leasing',
            'leasing_net': _dec(m.group('leasing')),
            'service_net': _dec(m.group('service')),
            'invoice_bukrs': _kaz_invoice_bukrs_from_text(text),
        })
    if not out:
        raise RuntimeError("Aus der Kazenmaier-PDF konnten keine Bike-Leasing-Summenpositionen erkannt werden.")
    global _KAZENMAIER_LAST_PARSED_POSITIONS
    _KAZENMAIER_LAST_PARSED_POSITIONS = out
    return out


def _kaz_bukrs_from_header(value):
    n = _norm(value)
    if 'DIGITAL' in n or n == 'IDG': return 'IDG'
    if 'MARKETINGSERVICES' in n or n == 'IMS': return 'IMS'
    if 'SABU' in n: return 'SABU'
    return 'IDE'


def _kaz_name_score(a,b):
    try:
        return _afi471_similarity(a,b)
    except Exception:
        import difflib
        aa=_norm(a); bb=_norm(b)
        return difflib.SequenceMatcher(None,aa,bb).ratio() if aa and bb else 0.0


def _kaz_load_bike_assignments(path):
    try:
        mtime = os.path.getmtime(path)
    except Exception as exc:
        raise RuntimeError("Kontierungszuordnung nicht gefunden: " + str(path)) from exc
    if _KAZENMAIER_BIKE_CACHE.get('path') == path and _KAZENMAIER_BIKE_CACHE.get('mtime') == mtime and _KAZENMAIER_BIKE_CACHE.get('items'):
        return _KAZENMAIER_BIKE_CACHE
    items=[]; gl_map={}
    try:
        from openpyxl import load_workbook
        wb=load_workbook(path,data_only=True,read_only=True)
        if 'Sachkonten' in wb.sheetnames and '_afi463_parse_sachkonten' in globals():
            gl_map = _afi463_parse_sachkonten(wb['Sachkonten'])
        if 'Bike' in wb.sheetnames:
            ws=wb['Bike']
            # Erwarteter Aufbau: je Organisation zwei Spalten Name/KST; Organisation steht in Zeile 1 oder 2 ueber dem Block.
            for c in range(1, ws.max_column + 1):
                header = _clean(ws.cell(1,c).value) or _clean(ws.cell(2,c).value)
                label = _clean(ws.cell(2,c).value) or _clean(ws.cell(3,c).value)
                if _norm(label) != 'NAME':
                    continue
                kst_col = c + 1
                bukrs = _kaz_bukrs_from_header(header or _clean(ws.cell(1,c).value))
                for r in range(3, ws.max_row + 1):
                    name = _clean(ws.cell(r,c).value)
                    kst = _clean(ws.cell(r,kst_col).value)
                    if not name or _norm(name) == 'NAME':
                        continue
                    if re.fullmatch(r"\d+\.0", kst): kst = kst[:-2]
                    items.append({'name':name, 'name_norm':_norm(name), 'bukrs':bukrs, 'kst':kst})
        try: wb.close()
        except Exception: pass
    except Exception as exc:
        raise RuntimeError("Bike-Leasing-Zuordnung konnte nicht gelesen werden: " + str(exc))
    _KAZENMAIER_BIKE_CACHE.update({'path':path,'mtime':mtime,'items':items,'gl':gl_map})
    return _KAZENMAIER_BIKE_CACHE


def _kaz_find_bike_assignment(items, name, preferred_bukrs='IDE'):
    target=_norm(name)
    if not target:
        return None, 0.0, 'leer'
    # 1. Exakt innerhalb der bevorzugten Organisation.
    for it in items:
        if it.get('bukrs') == preferred_bukrs and it.get('name_norm') == target:
            return it, 1.0, 'exakt'
    # 2. Exakt ueber alle Organisationen.
    exact=[it for it in items if it.get('name_norm') == target]
    if len(exact)==1:
        return exact[0], 1.0, 'exakt_orguebergreifend'
    if len(exact)>1:
        preferred=[it for it in exact if it.get('bukrs') == preferred_bukrs]
        return (preferred[0] if preferred else exact[0]), 1.0, 'exakt_mehrfach'
    # 3. Fuzzy normal, bevorzugte Organisation mit leichter Gewichtung.
    scored=[]
    for it in items:
        score=_kaz_name_score(name, it.get('name',''))
        if it.get('bukrs') == preferred_bukrs:
            score += 0.015
        if score >= 0.86:
            scored.append((score,it))
    scored.sort(key=lambda x:x[0], reverse=True)
    if not scored:
        return None, 0.0, 'nicht_gefunden'
    if len(scored)>1 and scored[0][0]-scored[1][0] < 0.025:
        return None, scored[0][0], 'mehrdeutig'
    return scored[0][1], scored[0][0], 'fuzzy'


def _kaz_gl_bike_leasing(cache, bukrs):
    glmap = (cache or {}).get('gl', {})
    for b in (bukrs, 'IDE', 'IDG', 'IMS', 'SABU'):
        m = glmap.get(b, {}) if isinstance(glmap, dict) else {}
        gl = m.get(_norm('Bike Leasing'), '') or m.get(_norm('Kazenmaier'), '')
        if gl:
            return gl
    return '416000'


def _kaz_create_export(assignment_path, invoice_path, export_path, config):
    paths = _afi480_split_invoice_paths(invoice_path) if '_afi480_split_invoice_paths' in globals() else ([invoice_path] if invoice_path else [])
    if not paths:
        raise RuntimeError('Bitte eine Kazenmaier-Rechnung auswählen.')
    cache = _kaz_load_bike_assignments(assignment_path)
    groups = OrderedDict()
    warnings=[]; fuzzy=[]; total=Decimal('0.00'); count_positions=0
    for path in paths:
        for pos in _kaz_extract_pdf_positions(path):
            count_positions += 1
            invoice_bukrs = pos.get('invoice_bukrs') or (config or {}).get('booking_circle') or 'IDE'
            ass, score, how = _kaz_find_bike_assignment(cache.get('items', []), pos.get('driver',''), invoice_bukrs)
            cc = (ass or {}).get('kst','') if ass else ''
            # Keine Intercompany-Weiterberechnung: es wird immer die persoenliche Bike-KST aus der Bike-Zuordnung verwendet.
            gl = _kaz_gl_bike_leasing(cache, invoice_bukrs)
            driver = _clean(pos.get('driver',''))
            if not ass:
                warnings.append(f"{driver}: keine eindeutige Bike-Leasing-KST-Zuordnung ({how})")
            elif how != 'exakt':
                fuzzy.append(f"{driver}: Bike-KST per {how} -> {ass.get('name','')} / {ass.get('kst','')} / {ass.get('bukrs','')} (Score {score:.2f})")
            tax = 'VD'
            amount = Decimal(pos.get('amount','0'))
            total += amount
            key = (_norm(driver), tax, gl, cc)
            if key not in groups:
                groups[key]={'TEXT':_clean(f"Bike Leasing {driver}"),'amount':Decimal('0.00'),'TAX_CODE':tax,'GL_ACCOUNT':gl,'COSTCENTER':cc,'ORDERID':'','driver_norm':_norm(driver)}
            groups[key]['amount'] += amount
    ordered=sorted(groups.values(), key=lambda g:g.get('driver_norm',''))
    os.makedirs(os.path.dirname(os.path.abspath(export_path)) or '.', exist_ok=True)
    export_total=Decimal('0.00')
    with open(export_path,'w',encoding='utf-8-sig',newline='') as f:
        writer=csv.DictWriter(f, fieldnames=UPLOAD_COLUMNS, delimiter=';', extrasaction='ignore')
        ...
        for g in ordered:
            ...
            export_total += amount
            text=g.get('TEXT','')
            if '_ascii_umlauts' in globals(): text=_ascii_umlauts(text)
            writer.writerow({'TEXT':text,'PRICE':_fmt(amount),'PRICE_UNIT':'1','QUANTITY':'1','UNIT':'ST','NET_VALUE':_fmt(amount),'TAX_CODE':g.get('TAX_CODE','VD'),'GL_ACCOUNT':g.get('GL_ACCOUNT',''),'COSTCENTER':g.get('COSTCENTER',''),'ORDERID':g.get('ORDERID','')})
    target=total.quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
    return {'rows':len(ordered),'export_path':export_path,'invoice_net_raw_total':_fmt(total),'export_net_total':_fmt(export_total),'net_rounding_difference':_fmt(export_total-target),'unique_drivers':len(ordered),'unique_keys':0,'missing_template':warnings,'unknown_tax':[],'empty_assignment':[],'name_fallback_matches':fuzzy,'foreign_gross':[],'kazenmaier_positions':count_positions,'kazenmaier_personal_kst':True,'kazenmaier_intercompany_ignored':True}


def _kaz428_supplier_from_invoice_name(invoice_path):
    name=_norm(os.path.basename(invoice_path or ''))
    if 'KAZENMAIER' in name:
        return 'KAZENMAIER'
    try:
        if os.path.splitext(invoice_path or '')[1].lower()=='.pdf' and _kaz_is_invoice_path(invoice_path):
            return 'KAZENMAIER'
    except Exception:
        pass
    return ''
...
_fm_supplier_from_invoice_name_before_v0484_kazenmaier = _fm_supplier_from_invoice_name if '_fm_supplier_from_invoice_name' in globals() else None

def _fm_supplier_from_invoice_name(invoice_path):
    kaz=_kaz428_supplier_from_invoice_name(invoice_path)
    if kaz:
        return kaz
    return _fm_supplier_from_invoice_name_before_v0484_kazenmaier(invoice_path) if _fm_supplier_from_invoice_name_before_v0484_kazenmaier else ''


_fm_short_vendor_before_v0484_kazenmaier = _fm_short_vendor if '_fm_short_vendor' in globals() else None

def _fm_short_vendor(path):
    if _kaz428_supplier_from_invoice_name(path):
        return 'Kazenmaier'
    return _fm_short_vendor_before_v0484_kazenmaier(path) if _fm_short_vendor_before_v0484_kazenmaier else (os.path.splitext(os.path.basename(path or 'Rechnung'))[0] or 'Rechnung')


_parse_pdf_invoice_positions_before_v0484_kazenmaier = _parse_pdf_invoice_positions

def _parse_pdf_invoice_positions(path, global_prefix):
    try:
        if _kaz_is_invoice_path(path):
            return _kaz_extract_pdf_positions(path)
    except Exception as exc:
        try:
            if 'KAZENMAIER' in _norm(os.path.basename(path or '')):
                raise exc
            ...
            raise
    return _parse_pdf_invoice_positions_before_v0484_kazenmaier(path, global_prefix)


_create_supplier_upload_csv_before_v0484_kazenmaier = create_supplier_upload_csv

def create_supplier_upload_csv(assignment_path, invoice_path, export_path, config):
    paths = _afi480_split_invoice_paths(invoice_path) if '_afi480_split_invoice_paths' in globals() else ([invoice_path] if invoice_path else [])
    try:
        if paths and all(os.path.splitext(p)[1].lower()=='.pdf' and _kaz_is_invoice_path(p) for p in paths):
            cfg=dict(config or {})
            cfg['global_prefix']='Bike Leasing'
            cfg['supplier']='Kazenmaier'
            return _kaz_create_export(assignment_path, invoice_path, export_path, cfg)
        return _create_supplier_upload_csv_before_v0484_kazenmaier(assignment_path, invoice_path, export_path, config)
    finally:
        if '_AFI478_CONTEXT' in globals():
            _AFI478_CONTEXT.update({'booking_circle':'IDE','intercompany':True,'assignment_path':''})
...
try:
    if 'Kazenmaier' not in SUPPLIER_OPTIONS:
        SUPPLIER_OPTIONS = list(SUPPLIER_OPTIONS) + ['Kazenmaier']
    if 'Bike Leasing' not in COST_TYPE_OPTIONS:
        COST_TYPE_OPTIONS = list(COST_TYPE_OPTIONS) + ['Bike Leasing']
except Exception:
    pass

# ------------------------------------------------------------------
# AFI_UPLOAD_KAZENMAIER_UST_SAMMELPOSTEN_V0485
# Datum: 2026-07-09
# Zweck:
# - Kazenmaier bildet eine Bike-Leasing-Ausnahme: zusaetzlich zu den Bike-Leasing-
#   Kosten je Person wird ein Sammelposten fuer die ausgewiesene Mehrwertsteuer gebucht.
# - Bike-Leasing-Kosten je Person werden ohne automatische Steuerlogik als V0 exportiert,
#   weil die USt separat auf das Sachkonto 154000 gebucht wird.
# - USt-Sammelposten: ohne Kostenstelle / ohne IA auf Sachkonto UST Bike Leasing bzw. 154000.
# ------------------------------------------------------------------
AFI_UPLOAD_KAZENMAIER_UST_SAMMELPOSTEN_VERSION = "0.485"


def _kaz485_extract_vat_total(path):
    text = _extract_pdf_text(path)
    compact = " ".join(text.replace("\n", " ").split())
    # Beispiel Kazenmaier: "Mehrwertsteuer A = 19,00% 1.554,41 EUR"
    patterns = [
        r"Mehrwertsteuer\s+A\s*=\s*19,00%\s+(\d{1,3}(?:\.\d{3})*,\d{2})\s*EUR",
        r"Mehrwertsteuer\s+.*?19,00%\s+(\d{1,3}(?:\.\d{3})*,\d{2})\s*EUR",
        r"Umsatzsteuer\s+.*?19,00%\s+(\d{1,3}(?:\.\d{3})*,\d{2})\s*EUR",
    ]
    for pat in patterns:
...
def _kaz485_gl_ust_bike_leasing(cache, bukrs):
    glmap = (cache or {}).get('gl', {})
    for b in (bukrs, 'IDE', 'IDG', 'IMS', 'SABU'):
        m = glmap.get(b, {}) if isinstance(glmap, dict) else {}
        gl = m.get(_norm('UST Bike Leasing'), '') or m.get(_norm('USt Bike Leasing'), '') or m.get(_norm('Mehrwertsteuer Bike Leasing'), '')
        if gl:
            return gl
    return '154000'


_kaz_create_export_before_v0485 = _kaz_create_export if '_kaz_create_export' in globals() else None
...
def _kaz_create_export(assignment_path, invoice_path, export_path, config):
    ...
    cache = _kaz_load_bike_assignments(assignment_path)
    groups = OrderedDict()
    warnings=[]; fuzzy=[]; total_net=Decimal('0.00'); total_vat=Decimal('0.00'); count_positions=0
    invoice_bukrs_for_vat = (config or {}).get('booking_circle') or 'IDE'
    for path in paths:
        try:
            total_vat += _kaz485_extract_vat_total(path)
        except Exception:
            pass
        for pos in _kaz_extract_pdf_positions(path):
            count_positions += 1
            invoice_bukrs = pos.get('invoice_bukrs') or (config or {}).get('booking_circle') or 'IDE'
            invoice_bukrs_for_vat = invoice_bukrs or invoice_bukrs_for_vat
            ass, score, how = _kaz_find_bike_assignment(cache.get('items', []), pos.get('driver',''), invoice_bukrs)
            cc = (ass or {}).get('kst','') if ass else ''
            gl = _kaz_gl_bike_leasing(cache, invoice_bukrs)
            driver = _clean(pos.get('driver',''))
            if not ass:
                ...
                fuzzy.append(f"{driver}: Bike-KST per {how} -> {ass.get('name','')} / {ass.get('kst','')} / {ass.get('bukrs','')} (Score {score:.2f})")
            # Kazenmaier: USt wird separat als Sammelposten gebucht. Daher Kostenzeilen ohne automatische Steuer: V0.
            tax = 'V0'
            amount = Decimal(pos.get('amount','0'))
            total_net += amount
            key = (_norm(driver), tax, gl, cc)
            ...
            groups[key]['amount'] += amount
    ordered=sorted(groups.values(), key=lambda g:g.get('driver_norm',''))
    vat_gl = _kaz485_gl_ust_bike_leasing(cache, invoice_bukrs_for_vat)
    if total_vat != 0:
        ordered.append({'TEXT':'UST Bike Leasing','amount':total_vat,'TAX_CODE':'V0','GL_ACCOUNT':vat_gl,'COSTCENTER':'','ORDERID':'','driver_norm':'ZZZ_UST'})
    os.makedirs(os.path.dirname(os.path.abspath(export_path)) or '.', exist_ok=True)
    export_total=Decimal('0.00')
    with open(export_path,'w',encoding='utf-8-sig',newline='') as f:
        writer=csv.DictWriter(f, fieldnames=UPLOAD_COLUMNS, delimiter=';', extrasaction='ignore')
        ...
        for g in ordered:
            ...
            text=g.get('TEXT','')
            if '_ascii_umlauts' in globals(): text=_ascii_umlauts(text)
            writer.writerow({'TEXT':text,'PRICE':_fmt(amount),'PRICE_UNIT':'1','QUANTITY':'1','UNIT':'ST','NET_VALUE':_fmt(amount),'TAX_CODE':g.get('TAX_CODE','V0'),'GL_ACCOUNT':g.get('GL_ACCOUNT',''),'COSTCENTER':g.get('COSTCENTER',''),'ORDERID':g.get('ORDERID','')})
    target=(total_net + total_vat).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
    return {'rows':len(ordered),'export_path':export_path,'invoice_net_raw_total':_fmt(total_net),'invoice_vat_total':_fmt(total_vat),'invoice_gross_total':_fmt(total_net+total_vat),'export_net_total':_fmt(export_total),'net_rounding_difference':_fmt(export_total-target),'unique_drivers':len(groups),'unique_keys':0,'missing_template':warnings,'unknown_tax':[],'empty_assignment':[],'name_fallback_matches':fuzzy,'foreign_gross':[],'kazenmaier_positions':count_positions,'kazenmaier_ust_sammelposten':bool(total_vat),'kazenmaier_ust_gl':vat_gl,'kazenmaier_personal_kst':True,'kazenmaier_intercompany_ignored':True}

# ------------------------------------------------------------------
# AFI_UPLOAD_KAZENMAIER_UST_TAXCODE_VD_V0486
# Datum: 2026-07-09
# Zweck:
# - Kazenmaier-USt-Sammelposten bleibt auf Sachkonto 154000 ohne KST/IA.
# - Da 154000 ein reines Vorsteuerkonto ist, bekommt nur diese MwSt-Position TAX_CODE = VD.
# - Bike-Leasing-Kostenpositionen mit persoenlicher KST bleiben weiterhin TAX_CODE = V0.
# ------------------------------------------------------------------
AFI_UPLOAD_KAZENMAIER_UST_TAXCODE_VD_VERSION = "0.486"

_kaz_create_export_before_v0486 = _kaz_create_export if '_kaz_create_export' in globals() else None
...
def _kaz_create_export(assignment_path, invoice_path, export_path, config):
    ...
    for path in paths:
        ...
        for pos in _kaz_extract_pdf_positions(path):
            ...
            if not ass:
                ...
                fuzzy.append(f"{driver}: Bike-KST per {how} -> {ass.get('name','')} / {ass.get('kst','')} / {ass.get('bukrs','')} (Score {score:.2f})")
            # Kazenmaier-Kostenpositionen: persoenliche KST + Bike-Leasing-Sachkonto, Tax Code bleibt V0.
            tax = 'V0'
            amount = Decimal(pos.get('amount','0'))
            total_net += amount
            key = (_norm(driver), tax, gl, cc)
    ...
    vat_gl = _kaz485_gl_ust_bike_leasing(cache, invoice_bukrs_for_vat)
    if total_vat != 0:
        # MwSt-Sammelposten: Vorsteuerkonto 154000 => TAX_CODE VD, keine Kostenstelle/IA.
        ordered.append({'TEXT':'UST Bike Leasing','amount':total_vat,'TAX_CODE':'VD','GL_ACCOUNT':vat_gl,'COSTCENTER':'','ORDERID':'','driver_norm':'ZZZ_UST'})
    os.makedirs(os.path.dirname(os.path.abspath(export_path)) or '.', exist_ok=True)
    export_total=Decimal('0.00')
    with open(export_path,'w',encoding='utf-8-sig',newline='') as f:
        writer=csv.DictWriter(f, fieldnames=UPLOAD_COLUMNS, delimiter=';', extrasaction='ignore')
        ...
        for g in ordered:
            ...
            writer.writerow({'TEXT':text,'PRICE':_fmt(amount),'PRICE_UNIT':'1','QUANTITY':'1','UNIT':'ST','NET_VALUE':_fmt(amount),'TAX_CODE':g.get('TAX_CODE','V0'),'GL_ACCOUNT':g.get('GL_ACCOUNT',''),'COSTCENTER':g.get('COSTCENTER',''),'ORDERID':g.get('ORDERID','')})
    target=(total_net + total_vat).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
    return {'rows':len(ordered),'export_path':export_path,'invoice_net_raw_total':_fmt(total_net),'invoice_vat_total':_fmt(total_vat),'invoice_gross_total':_fmt(total_net+total_vat),'export_net_total':_fmt(export_total),'net_rounding_difference':_fmt(export_total-target),'unique_drivers':len(groups),'unique_keys':0,'missing_template':warnings,'unknown_tax':[],'empty_assignment':[],'name_fallback_matches':fuzzy,'foreign_gross':[],'kazenmaier_positions':count_positions,'kazenmaier_ust_sammelposten':bool(total_vat),'kazenmaier_ust_gl':vat_gl,'kazenmaier_ust_tax_code':'VD' if total_vat else '','kazenmaier_cost_tax_code':'V0','kazenmaier_personal_kst':True,'kazenmaier_intercompany_ignored':True}

# ------------------------------------------------------------------
# AFI_UPLOAD_VODAFONE_STRICT_KST_MATCH_FIX_V0488
# Datum: 2026-07-09
# Zweck:
# - Vodafone darf bei fehlender Rufnummernzuordnung keine unsicheren Nachnamen-Fallbacks nutzen.
# - KST wird fuer Vodafone nur gesetzt, wenn die Rufnummer exakt gefunden wird oder der Name eindeutig belastbar ist.
# - Ambigue Kurz-/Nachnamen wie "Schneider, L." oder "Moser, Nina" werden nicht mehr falsch auf aehnliche Namen gemappt.
# - Sachkonto Vodafone wird trotzdem immer gesetzt.
# ------------------------------------------------------------------
AFI_UPLOAD_VODAFONE_STRICT_KST_MATCH_FIX_VERSION = "0.488"


def _afi488_norm_name(value):
    return _norm(value)


def _afi488_parse_invoice_name_from_text(text):
    s = _clean(text)
    m = re.search(r"0\d{2,5}/?\d{3,10}\s+(.*)$", s)
    return _clean(m.group(1)) if m else ""


def _afi488_name_parts(name):
    name = _clean(name)
    if not name:
        return "", ""
    if "," in name:
        last, first = [x.strip() for x in name.split(",", 1)]
        return _clean(last), _clean(first.replace(".", ""))
    parts = name.split()
    if len(parts) == 1:
        return parts[0], ""
...
def _afi488_entry_first_last(entry):
    return _clean((entry or {}).get('first','')), _clean((entry or {}).get('last',''))


def _afi488_strong_name_entry(entries, invoice_name):
    target = _afi488_norm_name(invoice_name)
    last, first = _afi488_name_parts(invoice_name)
    nlast = _afi488_norm_name(last)
    nfirst = _afi488_norm_name(first)
    if not target and not nlast:
        return None, 'leer'
    # 1. Vollname exakt oder eindeutig in normaler / gedrehter Reihenfolge.
    exact=[]
    for e in entries or []:
        full = e.get('name_norm','')
        alt = e.get('alt_name_norm','')
        if target and target in (full, alt):
            exact.append(e)
    if len(exact)==1:
        return exact[0], 'vollname_exakt'
    if len(exact)>1:
        return None, 'vollname_mehrdeutig'
    # 2. Nachname + ausgeschriebener Vorname oder eindeutige Initiale.
    if nlast and nfirst:
        candidates=[]
        for e in entries or []:
            ef, el = _afi488_entry_first_last(e)
            if _afi488_norm_name(el) != nlast:
                continue
            nef = _afi488_norm_name(ef)
            if not nef:
                continue
            if nef == nfirst or nef.startswith(nfirst) or (len(nfirst)==1 and nef.startswith(nfirst)):
                candidates.append(e)
        if len(candidates)==1:
            return candidates[0], 'nachname_vorname_initiale'
        if len(candidates)>1:
            return None, 'nachname_vorname_mehrdeutig'
    # 3. Nur Nachname: nur wenn exakt eine einzige Zuordnung mit diesem Nachnamen existiert.
    if nlast and not nfirst:
        candidates=[e for e in (entries or []) if _afi488_norm_name(e.get('last','')) == nlast]
        if len(candidates)==1:
            return candidates[0], 'nachname_eindeutig'
        if len(candidates)>1:
            return None, 'nachname_mehrdeutig'
    return None, 'nicht_gefunden'


def _afi488_vodafone_gl(assignment_path, bukrs='IDG'):
    try:
        cache = _afi463_load_general_overview(assignment_path)
        glmap = (cache.get('gl') or {}).get(_afi478_bukrs_label(bukrs) if '_afi478_bukrs_label' in globals() else bukrs, {})
        return glmap.get(_norm('Vodafone'), '') or glmap.get(_norm('Mobilfunk/Festnetz'), '') or '428300'
    except Exception:
        return '428300'


def _afi488_is_vodafone(invoice_path):
    try:
        if 'VODAFONE' in _norm(os.path.basename(invoice_path or '')):
            return True
        return _is_vodafone_pdf_text(_extract_pdf_text(invoice_path)) if '_is_vodafone_pdf_text' in globals() else False
    except Exception:
        return False


def _afi488_apply_strict_vodafone_matching(export_path, assignment_path, invoice_path, config):
    if not export_path or not os.path.isfile(export_path) or not _afi488_is_vodafone(invoice_path):
        return []
    try:
        entries = load_assignment_entries(assignment_path)
    except Exception:
        entries = []
    selected_bukrs = (config or {}).get('booking_circle') or 'IDG'
    fallback_gl = _afi488_vodafone_gl(assignment_path, selected_bukrs)
    phone_re = re.compile(r'0\d{2,5}/?\d{3,10}')
    with open(export_path, 'r', encoding='utf-8-sig', newline='') as f:
        reader = csv.DictReader(f, delimiter=';')
        fieldnames = reader.fieldnames or UPLOAD_COLUMNS
        rows = list(reader)
    ...
    for row in rows:
        ...
        if not m:
            continue
        phone = m.group(0)
        invoice_name = _afi488_parse_invoice_name_from_text(text)
        entry = _afi487_find_phone_entry(entries, phone) if '_afi487_find_phone_entry' in globals() else None
        how = 'rufnummer_exakt' if entry else ''
        if not entry:
            entry, how = _afi488_strong_name_entry(entries, invoice_name)
        before = {'GL_ACCOUNT':row.get('GL_ACCOUNT',''), 'COSTCENTER':row.get('COSTCENTER',''), 'ORDERID':row.get('ORDERID','')}
        if entry:
            gl, cc, ia = _select_assignment_values(entry, 'MOBILFUNK', 'Vodafone')
            row['GL_ACCOUNT'] = gl or fallback_gl
            row['COSTCENTER'] = cc or ''
            row['ORDERID'] = ia or ''
        else:
            # Unsichere Fallbacks entfernen: Sachkonto bleibt, KST/IA bewusst leer.
            row['GL_ACCOUNT'] = row.get('GL_ACCOUNT') or fallback_gl
            row['COSTCENTER'] = ''
            row['ORDERID'] = ''
        after = {'GL_ACCOUNT':row.get('GL_ACCOUNT',''), 'COSTCENTER':row.get('COSTCENTER',''), 'ORDERID':row.get('ORDERID','')}
        if before != after:
            corrections.append({'phone':phone, 'name':invoice_name, 'how':how, 'before':before, 'after':after})
    if corrections:
        with open(export_path, 'w', encoding='utf-8-sig', newline='') as f:
            writer = csv.DictWriter(f, fieldnames=fieldnames, delimiter=';', extrasaction='ignore')
            writer.writeheader(); writer.writerows(rows)
...
_create_supplier_upload_csv_before_v0488_vodafone_strict = create_supplier_upload_csv

def create_supplier_upload_csv(assignment_path, invoice_path, export_path, config):
    result = _create_supplier_upload_csv_before_v0488_vodafone_strict(assignment_path, invoice_path, export_path, config)
    try:
        corrections = _afi488_apply_strict_vodafone_matching(export_path, assignment_path, invoice_path, config)
        if isinstance(result, dict):
            result['vodafone_strict_kst_corrections'] = corrections
            result['vodafone_strict_kst_corrections_count'] = len(corrections)
    except Exception:
        pass
    return result

# ------------------------------------------------------------------
# AFI_UPLOAD_VODAFONE_PHONE_ONLY_KST_MATCH_V0489
# Datum: 2026-07-09
# Zweck:
# - Vodafone-KST-Zuordnung erfolgt fachlich ueber die Rufnummer, nicht ueber den Namen.
# - Namen in Vodafone-Rechnungen koennen Schreibfehler enthalten oder nur Initialien tragen.
# - Deshalb wird der Name nur noch fuer den Exporttext verwendet, aber nicht mehr fuer die KST-Ermittlung.
# - Rufnummernvergleich ist gegen Schreibweisen wie 0151/..., 0151..., 49..., 0049... robust.
# ------------------------------------------------------------------
AFI_UPLOAD_VODAFONE_PHONE_ONLY_KST_MATCH_VERSION = "0.489"


def _afi489_phone_variants(value):
    d = _digits_only(value) if '_digits_only' in globals() else re.sub(r'\D', '', str(value or ''))
    variants = []
    def add(x):
        x = str(x or '')
        if x and x not in variants:
            variants.append(x)
    add(d)
    if d.startswith('0049') and len(d) > 4:
        add('0' + d[4:])
        add(d[4:])
    if d.startswith('49') and len(d) > 2:
        add('0' + d[2:])
        add(d[2:])
...
def _afi489_phone_equal(a, b):
    av = set(_afi489_phone_variants(a))
    bv = set(_afi489_phone_variants(b))
    return bool(av and bv and av.intersection(bv))


def _afi489_find_phone_entry(entries, phone):
    phone_vars = set(_afi489_phone_variants(phone))
    if not phone_vars:
        return None, 'leer'
    exact = []
    for e in entries or []:
        if e.get('identifier_type') != 'PHONE':
            continue
        if phone_vars.intersection(set(_afi489_phone_variants(e.get('identifier','')))):
            exact.append(e)
    if len(exact) == 1:
        return exact[0], 'rufnummer_exakt'
    if len(exact) > 1:
        return None, 'rufnummer_mehrdeutig'
    # Fallback nur ueber eindeutige Endziffern, wenn die Schreibweisen unterschiedlich gekuerzt sind.
    pd = max(phone_vars, key=len)
    suffix = []
    for e in entries or []:
        if e.get('identifier_type') != 'PHONE':
            continue
        ev = _afi489_phone_variants(e.get('identifier',''))
        for ed in ev:
            if len(pd) >= 8 and len(ed) >= 8 and (pd.endswith(ed) or ed.endswith(pd)):
                suffix.append(e)
                break
    if len(suffix) == 1:
        return suffix[0], 'rufnummer_suffix_eindeutig'
    if len(suffix) > 1:
        return None, 'rufnummer_suffix_mehrdeutig'
    return None, 'rufnummer_nicht_gefunden'


def _afi489_apply_vodafone_phone_only_kst(export_path, assignment_path, invoice_path, config):
    if not export_path or not os.path.isfile(export_path):
        return []
    try:
        is_vodafone = 'VODAFONE' in _norm(os.path.basename(invoice_path or ''))
        if not is_vodafone and '_is_vodafone_pdf_text' in globals():
            try: is_vodafone = _is_vodafone_pdf_text(_extract_pdf_text(invoice_path))
            except Exception: is_vodafone = False
        if not is_vodafone:
            return []
        entries = load_assignment_entries(assignment_path)
        selected_bukrs = (config or {}).get('booking_circle') or 'IDG'
        fallback_gl = _afi488_vodafone_gl(assignment_path, selected_bukrs) if '_afi488_vodafone_gl' in globals() else '428300'
        phone_re = re.compile(r'0\d{2,5}/?\d{3,10}|(?:0049|49)\d{8,13}')
        with open(export_path, 'r', encoding='utf-8-sig', newline='') as f:
            reader = csv.DictReader(f, delimiter=';')
            fieldnames = reader.fieldnames or UPLOAD_COLUMNS
            rows = list(reader)
        ...
        for row in rows:
            ...
            if not m:
                continue
            phone = m.group(0)
            entry, how = _afi489_find_phone_entry(entries, phone)
            before = {'GL_ACCOUNT':row.get('GL_ACCOUNT',''), 'COSTCENTER':row.get('COSTCENTER',''), 'ORDERID':row.get('ORDERID','')}
            if entry:
                gl, cc, ia = _select_assignment_values(entry, 'MOBILFUNK', 'Vodafone')
                row['GL_ACCOUNT'] = gl or fallback_gl
                ...
                # Keine Rufnummernzuordnung => keine KST setzen. Namen sind bei Vodafone keine KST-Quelle.
                row['GL_ACCOUNT'] = row.get('GL_ACCOUNT') or fallback_gl
                row['COSTCENTER'] = ''
                row['ORDERID'] = ''
            after = {'GL_ACCOUNT':row.get('GL_ACCOUNT',''), 'COSTCENTER':row.get('COSTCENTER',''), 'ORDERID':row.get('ORDERID','')}
            if before != after:
                corrections.append({'phone':phone, 'how':how, 'before':before, 'after':after, 'text':text})
        if corrections:
            with open(export_path, 'w', encoding='utf-8-sig', newline='') as f:
                writer = csv.DictWriter(f, fieldnames=fieldnames, delimiter=';', extrasaction='ignore')
                writer.writeheader(); writer.writerows(rows)
...
_create_supplier_upload_csv_before_v0489_vodafone_phone_only = create_supplier_upload_csv

def create_supplier_upload_csv(assignment_path, invoice_path, export_path, config):
    result = _create_supplier_upload_csv_before_v0489_vodafone_phone_only(assignment_path, invoice_path, export_path, config)
    try:
        corrections = _afi489_apply_vodafone_phone_only_kst(export_path, assignment_path, invoice_path, config)
        if isinstance(result, dict):
            result['vodafone_phone_only_kst_corrections'] = corrections
            result['vodafone_phone_only_kst_corrections_count'] = len(corrections)
    except Exception:
        pass
    return result

# ------------------------------------------------------------------
# AFI_UPLOAD_VODAFONE_PHONE_TABLES_DIRECT_FIX_V0492
# Datum: 2026-07-09
# Zweck: Vodafone-KST strikt ueber Rufnummern aus Blatt "Telefon" ermitteln.
#        Beide Tabellen werden explizit gelesen: Telekom B:G und Vodafone J:O.
# ------------------------------------------------------------------
AFI_UPLOAD_VODAFONE_PHONE_TABLES_DIRECT_FIX_VERSION = "0.492"

def _afi492_digits(value):
    return re.sub(r'\D','',str(value or ''))

def _afi492_clean_code(value):
    s=_clean(value) if '_clean' in globals() else str(value or '').strip()
    return s[:-2] if re.fullmatch(r'\d+\.0',s) else s

def _afi492_phone_keys(value):
    d=_afi492_digits(value); out=[]
    def add(x):
        if x and x not in out: out.append(x)
    add(d)
    if d.startswith('0049') and len(d)>4: add('0'+d[4:]); add(d[4:])
    if d.startswith('49') and len(d)>2: add('0'+d[2:]); add(d[2:])
    if d.startswith('0') and len(d)>1: add('49'+d[1:]); add('0049'+d[1:]); add(d[1:])
    return out

def _afi492_load_phone_tables(path):
    out={}
    try:
        from openpyxl import load_workbook
        wb=load_workbook(path,data_only=True,read_only=True)
        if 'Telefon' not in wb.sheetnames:
            try: wb.close()
            except Exception: pass
            return out
        ws=wb['Telefon']
        # Vodafone zuerst, dann Telekom als Reserve. Tabellen: J:O und B:G.
        for source,start in [('Vodafone',10),('Telekom',2)]:
            for r in range(3, ws.max_row+1):
                phone=_afi492_clean_code(ws.cell(r,start).value)
                if not _afi492_digits(phone): continue
                item={'phone':phone,'last':_afi492_clean_code(ws.cell(r,start+1).value),'first':_afi492_clean_code(ws.cell(r,start+2).value),'firma':_afi492_clean_code(ws.cell(r,start+3).value),'kst':_afi492_clean_code(ws.cell(r,start+4).value),'ia':_afi492_clean_code(ws.cell(r,start+5).value),'source':source,'row':r}
                for key in _afi492_phone_keys(phone):
                    if key not in out or source=='Vodafone': out[key]=item
        try: wb.close()
        except Exception: pass
    except Exception:
        pass
...
def _afi492_find_phone(mapping, phone):
    for key in _afi492_phone_keys(phone):
        if key in mapping: return mapping[key]
    q=max(_afi492_phone_keys(phone) or [''], key=len)
    if len(q)<8: return None
    hits=[]
    for k,v in mapping.items():
        if len(k)>=8 and (k.endswith(q) or q.endswith(k)): hits.append(v)
    return hits[0] if len(hits)==1 else None

def _afi492_is_vodafone_invoice(path):
    try:
        return 'VODAFONE' in _norm(os.path.basename(path or '')) or ('_is_vodafone_pdf_text' in globals() and _is_vodafone_pdf_text(_extract_pdf_text(path)))
    except Exception:
        return False

def _afi492_vodafone_gl(path):
    try:
        from openpyxl import load_workbook
        wb=load_workbook(path,data_only=True,read_only=True)
        if 'Sachkonten' in wb.sheetnames:
            ...
            for r in range(1, ws.max_row+1):
                for c in [2,5,8,11]:
                    if 'VODAFONE' in _norm(ws.cell(r,c).value):
                        gl=_afi492_clean_code(ws.cell(r,c+1).value)
                        if gl:
                            try: wb.close()
                            except Exception: pass
                            return gl
...
def _afi492_apply_vodafone_kst(export_path, assignment_path, invoice_path):
    if not export_path or not os.path.isfile(export_path) or not _afi492_is_vodafone_invoice(invoice_path):
        return {'phone_rows':0,'matched':0,'missing':[],'updates':[]}
    mapping=_afi492_load_phone_tables(assignment_path); fallback_gl=_afi492_vodafone_gl(assignment_path)
    phone_re=re.compile(r'0\d{2,5}/?\d{3,10}|(?:0049|49)\d{8,13}')
    with open(export_path,'r',encoding='utf-8-sig',newline='') as f:
        reader=csv.DictReader(f,delimiter=';'); fieldnames=reader.fieldnames or UPLOAD_COLUMNS; rows=list(reader)
    phone_rows=matched=0; missing=[]; updates=[]
    for row in rows:
        m=phone_re.search(row.get('TEXT',''))
        if not m: continue
        phone_rows+=1; phone=m.group(0); hit=_afi492_find_phone(mapping, phone)
        before=(row.get('GL_ACCOUNT',''),row.get('COSTCENTER',''),row.get('ORDERID',''))
        row['GL_ACCOUNT']=row.get('GL_ACCOUNT') or fallback_gl
        if hit and hit.get('kst'):
            matched+=1; row['COSTCENTER']=hit.get('kst',''); row['ORDERID']=hit.get('ia','') or ''
        else:
            row['COSTCENTER']=''; row['ORDERID']=''; missing.append(phone)
        after=(row.get('GL_ACCOUNT',''),row.get('COSTCENTER',''),row.get('ORDERID',''))
        if before!=after: updates.append({'phone':phone,'before':before,'after':after,'found':bool(hit and hit.get('kst'))})
    if updates:
        with open(export_path,'w',encoding='utf-8-sig',newline='') as f:
            w=csv.DictWriter(f,fieldnames=fieldnames,delimiter=';',extrasaction='ignore'); w.writeheader(); w.writerows(rows)
    return {'phone_rows':phone_rows,'matched':matched,'missing':missing,'updates':updates}

_create_supplier_upload_csv_before_v0492_vodafone_phone_tables = create_supplier_upload_csv

def create_supplier_upload_csv(assignment_path, invoice_path, export_path, config):
    result=_create_supplier_upload_csv_before_v0492_vodafone_phone_tables(assignment_path, invoice_path, export_path, config)
    try:
        info=_afi492_apply_vodafone_kst(export_path, assignment_path, invoice_path)
        if isinstance(result,dict):
            result['vodafone_phone_table_rows']=info.get('phone_rows',0)
            result['vodafone_phone_table_matched']=info.get('matched',0)
            result['vodafone_phone_table_missing']=info.get('missing',[])
            result['vodafone_phone_table_updates_count']=len(info.get('updates',[]))
    except Exception: pass
    return result

# ------------------------------------------------------------------
# AFI_UPLOAD_KAZENMAIER_COST_TAXCODE_EMPTY_V0493
# Datum: 2026-07-10
# Zweck:
# - Kazenmaier Bike Leasing: Einzelpositionen haben kein Steuerkennzeichen.
# - Nur die MwSt-Sammelposition auf dem Vorsteuerkonto 154000 enthaelt TAX_CODE = VD.
# - Die vorhandene Rufnummern-/Vodafone-Logik bleibt unveraendert.
# ------------------------------------------------------------------
AFI_UPLOAD_KAZENMAIER_COST_TAXCODE_EMPTY_VERSION = "0.493"

_kaz_create_export_before_v0493 = _kaz_create_export if '_kaz_create_export' in globals() else None
...
def _kaz_create_export(assignment_path, invoice_path, export_path, config):
    ...
    for path in paths:
        ...
        for pos in _kaz_extract_pdf_positions(path):
            ...
            if not ass:
                ...
                fuzzy.append(f"{driver}: Bike-KST per {how} -> {ass.get('name','')} / {ass.get('kst','')} / {ass.get('bukrs','')} (Score {score:.2f})")
            # Fachlich ab v0.493: Kostenpositionen ohne Steuerkennzeichen. Nur MwSt-Sammelposten hat VD.
            tax = ''
            amount = Decimal(pos.get('amount','0'))
            total_net += amount
            key = (_norm(driver), tax, gl, cc)
    ...
    with open(export_path,'w',encoding='utf-8-sig',newline='') as f:
        ...
        for g in ordered:
            ...
            text=g.get('TEXT','')
            if '_ascii_umlauts' in globals(): text=_ascii_umlauts(text)
            writer.writerow({'TEXT':text,'PRICE':_fmt(amount),'PRICE_UNIT':'1','QUANTITY':'1','UNIT':'ST','NET_VALUE':_fmt(amount),'TAX_CODE':g.get('TAX_CODE',''),'GL_ACCOUNT':g.get('GL_ACCOUNT',''),'COSTCENTER':g.get('COSTCENTER',''),'ORDERID':g.get('ORDERID','')})
    target=(total_net + total_vat).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
    return {'rows':len(ordered),'export_path':export_path,'invoice_net_raw_total':_fmt(total_net),'invoice_vat_total':_fmt(total_vat),'invoice_gross_total':_fmt(total_net+total_vat),'export_net_total':_fmt(export_total),'net_rounding_difference':_fmt(export_total-target),'unique_drivers':len(groups),'unique_keys':0,'missing_template':warnings,'unknown_tax':[],'empty_assignment':[],'name_fallback_matches':fuzzy,'foreign_gross':[],'kazenmaier_positions':count_positions,'kazenmaier_ust_sammelposten':bool(total_vat),'kazenmaier_ust_gl':vat_gl,'kazenmaier_ust_tax_code':'VD' if total_vat else '','kazenmaier_cost_tax_code':'','kazenmaier_personal_kst':True,'kazenmaier_intercompany_ignored':True}

# ------------------------------------------------------------------
# AFI_UPLOAD_KAZENMAIER_FINAL_TAX_POSTPROCESS_V0494
# Datum: 2026-07-10
# Zweck:
# - Kazenmaier Bike Leasing final absichern:
#   * Einzelpositionen haben IMMER ein leeres Steuerkennzeichen.
#   * Die MwSt-Sammelposition wird IMMER angefuegt, wenn in der PDF MwSt ausgewiesen ist.
#   * Nur die MwSt-Sammelposition hat TAX_CODE = VD und GL_ACCOUNT = 154000 bzw. UST Bike Leasing.
# - Diese Nachbearbeitung greift nach allen vorhandenen Exportpfaden, damit keine aeltere Logik
#   wieder TAX_CODE = V0/VD auf Kostenpositionen setzt oder die MwSt-Zeile entfernt.
# ------------------------------------------------------------------
AFI_UPLOAD_KAZENMAIER_FINAL_TAX_POSTPROCESS_VERSION = "0.494"


def _kaz494_is_kazenmaier_invoice(invoice_path):
    try:
        paths = _afi480_split_invoice_paths(invoice_path) if '_afi480_split_invoice_paths' in globals() else ([invoice_path] if invoice_path else [])
        return bool(paths) and all(os.path.splitext(p)[1].lower()=='.pdf' and _kaz_is_invoice_path(p) for p in paths)
    except Exception:
        try:
            return 'KAZENMAIER' in _norm(os.path.basename(invoice_path or ''))
        except Exception:
            return False


def _kaz494_total_vat_from_paths(invoice_path):
    total = Decimal('0.00')
    paths = _afi480_split_invoice_paths(invoice_path) if '_afi480_split_invoice_paths' in globals() else ([invoice_path] if invoice_path else [])
    for p in paths:
        try:
            total += Decimal(_kaz485_extract_vat_total(p))
        except Exception:
            pass
    return total


def _kaz494_ust_gl(assignment_path, invoice_path, config):
    try:
        cache = _kaz_load_bike_assignments(assignment_path)
        bukrs = (config or {}).get('booking_circle') or 'IDE'
        try:
            paths = _afi480_split_invoice_paths(invoice_path) if '_afi480_split_invoice_paths' in globals() else ([invoice_path] if invoice_path else [])
            if paths:
                pos = _kaz_extract_pdf_positions(paths[0])
                if pos:
                    bukrs = pos[0].get('invoice_bukrs') or bukrs
        except Exception:
            pass
        return _kaz485_gl_ust_bike_leasing(cache, bukrs) or '154000'
    except Exception:
        return '154000'


def _kaz494_postprocess_export(export_path, assignment_path, invoice_path, config):
    if not export_path or not os.path.isfile(export_path) or not _kaz494_is_kazenmaier_invoice(invoice_path):
        return {'applied': False, 'reason': 'not_kazenmaier'}
    with open(export_path, 'r', encoding='utf-8-sig', newline='') as f:
        reader = csv.DictReader(f, delimiter=';')
        fieldnames = reader.fieldnames or UPLOAD_COLUMNS
        rows = list(reader)
    if 'TAX_CODE' not in fieldnames:
        fieldnames = list(fieldnames) + ['TAX_CODE']
    if 'TEXT' not in fieldnames:
        fieldnames = list(fieldnames) + ['TEXT']
    if 'GL_ACCOUNT' not in fieldnames:
        fieldnames = list(fieldnames) + ['GL_ACCOUNT']
    if 'COSTCENTER' not in fieldnames:
        fieldnames = list(fieldnames) + ['COSTCENTER']
    if 'ORDERID' not in fieldnames:
        fieldnames = list(fieldnames) + ['ORDERID']
    if 'NET_VALUE' not in fieldnames:
        fieldnames = list(fieldnames) + ['NET_VALUE']
    if 'PRICE' not in fieldnames:
        fieldnames = list(fieldnames) + ['PRICE']
    vat_total = _kaz494_total_vat_from_paths(invoice_path).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
    ust_norm = _norm('UST Bike Leasing')
    removed_ust = 0
    new_rows = []
    cleared_cost_tax = 0
    for row in rows:
        text_norm = _norm(row.get('TEXT',''))
        is_ust = text_norm == ust_norm or (row.get('GL_ACCOUNT','') == '154000' and 'BIKE' in text_norm and ('UST' in text_norm or 'MWST' in text_norm or 'MEHRWERTSTEUER' in text_norm))
        if is_ust:
            removed_ust += 1
            continue
        if row.get('TAX_CODE','') != '':
    ...
    ust_added = False
    if vat_total != 0:
        vat_gl = _kaz494_ust_gl(assignment_path, invoice_path, config)
        ust_row = {k:'' for k in fieldnames}
        ust_row.update({
            'TEXT':'UST Bike Leasing',
            'PRICE':_fmt(vat_total),
            'PRICE_UNIT':'1',
            'QUANTITY':'1',
            'UNIT':'ST',
            'NET_VALUE':_fmt(vat_total),
            'TAX_CODE':'VD',
            'GL_ACCOUNT':vat_gl or '154000',
            'COSTCENTER':'',
            'ORDERID':''
        })
        new_rows.append(ust_row)
        ust_added = True
    with open(export_path, 'w', encoding='utf-8-sig', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=fieldnames, delimiter=';', extrasaction='ignore')
        writer.writeheader()
        writer.writerows(new_rows)
    return {'applied': True, 'rows_before': len(rows), 'rows_after': len(new_rows), 'removed_existing_ust_rows': removed_ust, 'cleared_cost_tax_rows': cleared_cost_tax, 'ust_added': ust_added, 'vat_total': _fmt(vat_total)}


_create_supplier_upload_csv_before_v0494_kazenmaier_tax_postprocess = create_supplier_upload_csv

def create_supplier_upload_csv(assignment_path, invoice_path, export_path, config):
    result = _create_supplier_upload_csv_before_v0494_kazenmaier_tax_postprocess(assignment_path, invoice_path, export_path, config)
    try:
        info = _kaz494_postprocess_export(export_path, assignment_path, invoice_path, config)
        if isinstance(result, dict):
            result['kazenmaier_tax_postprocess_v0494'] = info
            if info.get('applied'):
                result['rows'] = info.get('rows_after', result.get('rows'))
                result['kazenmaier_ust_sammelposten'] = bool(info.get('ust_added'))
                result['kazenmaier_cost_tax_code'] = ''
                result['kazenmaier_ust_tax_code'] = 'VD' if info.get('ust_added') else ''
    except Exception as exc:
        if isinstance(result, dict):
            result['kazenmaier_tax_postprocess_v0494_error'] = str(exc)
    return result
