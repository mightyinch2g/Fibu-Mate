...
STANDARD_COLUMNS = [
    ...
    "EK Lieferant", "Menge gesamt", "Gesamtbetrag", "Modell", "MG-Nummer", "Warenempfänger", "Kundennummer IDE", "Ihre Filial-Nr.:"
]
DISPLAY_COLUMNS = [
...
def extract_store_number(full_text: str) -> str:
    """Extrahiert robust die Angabe zu 'Ihre Filial-Nr.:'.

    Nike-PDFs können nach dem Label zunächst Rechnungs-/Lieferdatum,
    Produktbereich und Kategorie ausgeben. Deshalb wird nicht mehr blind die
    erste Zahl nach dem Label genommen, sondern zeilenbasiert in den folgenden
    Zeilen nach einer plausiblen langen Filialnummer gesucht.
    """
    text = full_text or ""
    lines = [normalize_text(line).replace("**", "").strip() for line in text.splitlines()]
    lines = [line for line in lines if line]

    def is_plausible_store_number(candidate: str) -> bool:
        candidate = normalize_text(candidate)
        if not re.fullmatch(r"\d{8,20}", candidate):
            return False
        # Datumswerte oder kurze Tages-/Monatsbestandteile dürfen nicht als Filialnummer gelten.
        if re.fullmatch(r"\d{1,2}", candidate):
            return False
        return True

    for index, line in enumerate(lines):
        if re.search(r"Ihre\s+Filial\s*-?\s*Nr\.?", line, re.IGNORECASE):
            # 1) Falls die Nummer wirklich direkt in derselben Zeile steht.
            same_line_match = re.search(r"Ihre\s+Filial\s*-?\s*Nr\.?\s*:?\s*(\d{8,20})\b", line, re.IGNORECASE)
            if same_line_match and is_plausible_store_number(same_line_match.group(1)):
                return same_line_match.group(1)

            # 2) Robust in den Folgezeilen suchen. Beispiel: Label, dann Datum, Datum,
            # Produktbereich, Kategorie, erst danach die Filialnummer.
            for search_index in range(index + 1, min(index + 9, len(lines))):
                candidate_line = normalize_text(lines[search_index])
                if re.fullmatch(r"\d{2}\.\d{2}\.\d{4}", candidate_line):
                    continue
                for candidate in re.findall(r"\b\d{8,20}\b", candidate_line):
                    if is_plausible_store_number(candidate):
                        return candidate
            return ""
    return ""
...
def aggregate_pdf_data(doc):
    ...
    store_number = extract_store_number(full_text)
    ...
    if not any([order_numbers, models, brutto_values, discount_values, supplier_values, total_quantity, mg_number, member_name, customer_number_ide]):
        return None
    return [re_date, re_nr, join_unique(order_numbers), "", join_unique(brutto_values), join_unique(discount_values), join_unique(supplier_values), total_quantity, total_amount, join_unique(models), mg_number, member_name, customer_number_ide, store_number]
...
def write_cell_value(worksheet, row_index, column_index, value):
    ...
    value = normalize_text(value)
    if column_index in (1, 2, 3, 10, 11, 12, 13, 14):
        cell.value = value
        cell.number_format = "@"
        return
...
def open_filter_popup(app, state, filter_button, clear_filter_canvas_item):
    ...
    popup.geometry("1700x700")
    popup.minsize(1450, 590)
    ...
    title.grid(row=0, column=0, columnspan=column_count, sticky="w", pady=(0, 4))
    info = tk.Label(content, text="Spalte A bis N zeigt die spätere Excel-Reihenfolge. Kategorie anklicken, mit den Pfeilkacheln verschieben oder unter der Kategorie an-/abhaken.", font=("Segoe UI", 10), bg=COLOR_APP_BG, fg=COLOR_TEXT_SECONDARY)
    info.grid(row=1, column=0, columnspan=column_count, sticky="w", pady=(0, 14))
    category_frame = tk.Frame(content, bg=COLOR_APP_BG)
    category_frame.grid(row=2, column=0, columnspan=column_count, sticky="n", pady=(0, 10))
    ...
    def redraw_categories():
        ...
        if excluded_start < len(working_config["order"]):
            ...
            legend.grid(row=0, column=excluded_start, columnspan=max(1, column_count - excluded_start), sticky="w", padx=6, pady=(0, 2))
        ...
        if excluded_start < len(working_config["order"]):
            ...
            if separator_column < column_count:
    ...
    nav_frame.grid(row=3, column=0, columnspan=column_count, pady=(2, 18))
    ...
    footer_outer.grid(row=4, column=0, columnspan=column_count, pady=(0, 8))
