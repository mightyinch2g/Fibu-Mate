...
try:
    ...
    try: import compliance_common as cc
...
class YearlyCloseUI:
    ...
    def log_period_event(self, action, reason="", extra=None):
        ...
        try:
            if cc:
                event_type = "Zeitraum abgeschlossen" if action == "closed" else "Zeitraum wieder geöffnet" if action == "opened" else "Info"
                risk = "Info" if action == "closed" else "Kritisch" if action == "opened" else "Info"
                cc.log_audit(self.app, event_type, "Jahresabschluss", period_label(self.period), reason or event_type, risk, self.period, "", True)
                if action == "opened" and reason: cc.log_audit(self.app, "Begründung der Wiederöffnung", "Jahresabschluss", period_label(self.period), reason, "Hoch", self.period, "", True)
        except Exception: pass
    ...
    def log_change(self, action, task=None, field="", old="", new=""):
        ...
        try:
            if cc:
                event_type = "Änderungen nach Wiederöffnung" if after_reopen else ("Fälligkeit geändert" if "fällig" in str(field).lower() or "due" in str(field).lower() else "Aufgabe geändert")
                risk = "Kritisch" if after_reopen else ("Mittel" if event_type == "Fälligkeit geändert" else "Info")
                task_title = task.get("title", "") if isinstance(task, dict) else ""
                task_uid = task.get("task_uid", "") if isinstance(task, dict) else ""
                cc.log_audit(self.app, event_type, "Jahresabschluss", task_title or action, f"{action} | {field}: {old} → {new}", risk, self.period, task_uid, True)
        except Exception: pass
