    ...
    def log_period_event(self, action, reason="", extra=None):
        ...
        try:
            if cc:
                ...
                cc.log_audit(self.app, event_type, "Quartalsabschluss", period_label(self.period), reason or event_type, risk, self.period, "", True)
                if action == "opened" and reason: cc.log_audit(self.app, "Begründung der Wiederöffnung", "Quartalsabschluss", period_label(self.period), reason, "Hoch", self.period, "", True)
        except Exception: pass
    ...
    def log_change(self, action, task=None, field="", old="", new=""):
        ...
        try:
            if cc:
                ...
                task_title = task.get("title", "") if isinstance(task, dict) else ""
                task_uid = task.get("task_uid", "") if isinstance(task, dict) else ""
                cc.log_audit(self.app, event_type, "Quartalsabschluss", task_title or action, f"{action} | {field}: {old} → {new}", risk, self.period, task_uid, True)
        except Exception: pass
