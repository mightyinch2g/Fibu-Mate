...
class FiBuMateApp:
    def __init__(self):
        ...
        self.bump_version_once("2026-05-27_v04_scroll_audit_sync_fix", ["Globales Scroll-Verhalten korrigiert.", "Audit-Log-Pfad und Abschluss-Audit-Hooks ergänzt.", "Bidirektionale Monatsabschluss-Synchronisation für Z4/ZM/Z5a über MQ001/MQ002/MQ003 ergänzt."])
    ...
    def _is_descendant_of(self, widget, parent):
        ...
        return False
    def _canvas_has_vertical_overflow(self, canvas):
        try:
            bbox=canvas.bbox("all"); return bool(bbox and (bbox[3]-bbox[1]) > max(1, canvas.winfo_height()+2))
        except Exception: return False
    def _find_scroll_canvas_from_widget(self, widget):
        try:
            while widget is not None:
                if isinstance(widget, tk.Canvas) and self._canvas_has_vertical_overflow(widget): return widget
                widget = widget.master
        except Exception: pass
        canvas=getattr(self,"active_scroll_canvas",None)
        if canvas is not None and self._canvas_has_vertical_overflow(canvas):
            try:
                w=self.root.winfo_containing(self.root.winfo_pointerx(), self.root.winfo_pointery())
                if self._is_descendant_of(w, canvas): return canvas
            except Exception: pass
        return None
    def on_global_mousewheel(self, event):
        ...
        try:
            ...
            if cls in ("TCombobox","Combobox","Menu","Listbox") or "combobox" in str(widget).lower(): return "break"
        except Exception: pass
        target=self._find_scroll_canvas_from_widget(widget)
        if target is None: return "break"
        delta=int(-event.delta/120) if getattr(event,"delta",0) else 0
        if delta: target.yview_scroll(delta,"units")
        if hasattr(self,"_update_scroll_indicators"): self._update_scroll_indicators()
        return "break"
    ...
    def render_users_menu(self):
        ...
        def save_user_row(old_key, name_var, email_var, first_name_var=None):
            ...
            self.save_user_data(); self.log_audit_event("Benutzer geändert", "Benutzerverwaltung", "Benutzer gespeichert", old_key, "Mittel", public=False); messagebox.showinfo("FiBu Mate", "Benutzerdaten wurden gespeichert."); self.render_page()
        def delete_user(user_key):
            ...
            users.pop(user_key, None); self.save_user_data(); self.log_audit_event("Benutzer gelöscht", "Benutzerverwaltung", "Benutzer gelöscht", user_key, "Hoch", public=False); messagebox.showinfo("FiBu Mate", "Benutzer wurde gelöscht."); self.render_page()
        row = 2
        ...
        if self.can_create_users():
            ...
            def create_user():
                ...
                self.ensure_permissions_defaults(); self.save_user_data(); self.log_audit_event("Benutzer angelegt", "Benutzerverwaltung", "Benutzer angelegt", raw_name, "Mittel", public=False); messagebox.showinfo("FiBu Mate", f"Benutzer wurde angelegt:\n\n{raw_name}"); self.render_page()
    ...
    def render_permissions_menu(self):
        ...
        for key in sorted(users.keys()):
            ...
            def save_role(user_key=key, var=role_var):
                ...
                old_role = users[user_key].get("permission", ""); users[user_key]["permission"] = new_role; self.ensure_permissions_defaults(); self.save_user_data(); self.log_audit_event("Berechtigung geändert", "Berechtigungen", f"Berechtigung {user_key}", f"{old_role} → {new_role}", "Hoch", public=False); messagebox.showinfo("FiBu Mate", "Berechtigung wurde gespeichert."); self.render_page()
