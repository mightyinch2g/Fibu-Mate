...
class Tile(tk.Canvas):
    ...
    def draw(self):
        ...
        long_title = len(self.title) > 34
        title_y = y0 + (20 if long_title else 36)
        title_font = FONT_TILE_SMALL if long_title else self.title_font()
        icon_y = y0 + self.tile_height * (0.80 if long_title else 0.64)

        if self.lock_tile:
            self.create_text((x0 + x1) / 2, y0 + 22, text=self.title, anchor="n", fill=BLUE, font=self.title_font(), width=max(120, self.tile_width - 44), justify="center")
            ...
            if self.center_text:
                ...
                self.create_text((x0 + x1) / 2, title_y, text=self.title, anchor="n", fill="white", font=title_font, width=max(120, self.tile_width - 44), justify="center")
            if self.icon_type:
                self._draw_main_icon(self.icon_type, (x0 + x1) / 2, icon_y)
...
class FiBuMateApp:
    ...
    def draw_tile_icon_image(self, tile, icon_type, cx, cy):
        ...
        if icon_type == "pdf_xls":
            p1 = self.get_icon_photo("pdf", 38, 38)
            p2 = self.get_icon_photo("xls", 38, 38)
            if p1 and p2:
                tile.create_image(cx - 24, cy, image=p1)
                tile.create_image(cx + 24, cy, image=p2)
                return True
            return False
    ...
    def module_icon_type(self, module_id):
        ...
        if self.current_page == "closing_calendar":
            return "calendar"
        if module_id in ("monthly_close", "quarterly_close"):
            return "calendar"
