...
def make_button(app, text, command, width=22, height=2, icon_file=None):
    # Classic buttons kept for path selection, with optional format icon on the right.
    btn = tk.Button(app.root, text=text, font=("Segoe UI", 10, "bold"), width=width, height=height, command=command, bg=INTERSPORT_BLUE, fg="white", activebackground=INTERSPORT_BLUE_ACTIVE, activeforeground="white", cursor="hand2", bd=0, compound="right")
    if icon_file:
        photo = load_icon_photo(app.root, icon_file, 20, 20)
        if photo:
...
def render(app):
    ...
    folder_button = make_button(app, "PDF-Ordner auswählen", choose_folder, width=26, height=2, icon_file=PDF_ICON)
    save_button = make_button(app, "Excel-Speicherpfad wählen", choose_save_path, width=26, height=2, icon_file=XLS_ICON)
