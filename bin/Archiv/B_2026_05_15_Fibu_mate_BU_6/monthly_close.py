...
class MonthlyCloseUI:
    ...
    def create_attachment_button(self, parent, item, command):
        ...
        btn = tk.Button(frame, text="" if photo else "📎", image=photo, command=command, bg=parent.cget("bg"), fg=COLORS["blue"], bd=0, cursor="hand2")
        if photo:
            btn.image = photo
        btn.pack(side="left", padx=(2, 1))
        tk.Label(frame, text=str(self.attachment_count(item)), bg=parent.cget("bg"), fg=COLORS["blue"], font=("Segoe UI", 10, "bold")).pack(side="left", padx=(1, 2))
        return frame

    def draw_documentation_icon(self, canvas, has_documentation):
        canvas.delete("all")
        canvas.create_rectangle(1, 1, 31, 21, fill=COLORS["blue"], outline=COLORS["blue"])
        ...
        photo = self.get_close_icon_photo(icon_file, 18, 18)
        if photo:
            canvas.create_image(16, 11, image=photo)
            canvas.image = photo
            return
        if not has_documentation:
            canvas.create_text(16, 11, text="+", fill="white", font=("Segoe UI", 13, "bold"))
            return
        for y in (6, 11, 16):
            canvas.create_oval(6, y - 2, 9, y + 1, fill="white", outline="white")
            canvas.create_line(13, y, 26, y, fill="white", width=2, capstyle="round")

    def create_documentation_button(self, parent, item, title, parent_task=None):
        has_doc = bool(item.get("documentation", {}).get("path"))
        btn = tk.Canvas(parent, width=32, height=22, bg=COLORS["white"], highlightthickness=0, bd=0, cursor="hand2")
    ...
    def show_documentation_popup(self, item, title, parent_task=None):
        ...
        tk.Button(row, text="Dokumentation öffnen", command=lambda: self.open_attachment(path_var.get()), bg=COLORS["blue"], fg="white", bd=0, padx=12, pady=6, state="normal" if path_var.get() else "disabled").pack(side="left")
        tk.Label(row, textvariable=name_var, bg=COLORS["white"], fg=COLORS["text"], font=("Segoe UI", 10)).pack(side="left", padx=10)
        ...
        def choose_documentation():
            ...
            self.save()
            name_var.set(os.path.basename(selected)); path_var.set(selected)
            if self.selected_team:
                self.render_team_detail(self.selected_team)
            win.destroy()
        ...
        tk.Label(body, text="Hinweis: Die Dokumentation ist für Aufgabenbeschreibungen bzw. Leitfäden vorgesehen. Ergebnisse und Bearbeitungskommentare bitte unter Anlagen pflegen.", bg=COLORS["white"], fg=COLORS["text2"], font=("Segoe UI", 9), wraplength=620, justify="left").pack(anchor="w", padx=12, pady=(0, 10))
    ...
    def render_task_table(self, team):
        ...
        min_sizes = {0: 46, 1: 330, 2: 92, 3: 130, 4: 220, 5: 105, 6: 105, 7: 120, 8: 100, 9: 88, 10: 150}
    ...
    def render_task_row(self, table, row_idx, task, headers):
        ...
        tk.Label(table, text=task.get("owner"), bg=bg, fg=COLORS["text"], font=("Segoe UI", 10), padx=6, pady=6, anchor="center", justify="center").grid(row=row_idx, column=3, sticky="nsew", padx=1, pady=1)
