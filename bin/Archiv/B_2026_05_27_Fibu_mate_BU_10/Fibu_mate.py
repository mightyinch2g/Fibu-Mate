...
ICON_FILES = {
    ...
    "idea": "idea_lightbulb_icon_182987.ico",
...
OLD_ROLE_WAGNERM = "Wagnerm"
ROLE_WAGNERM = "System-Administrator"
SUPERUSER_KEY = "wagnerm"
ROLE_ORDER = [ROLE_STANDARD, ROLE_ADMIN, ROLE_WAGNERM]
ROLE_RANK = {ROLE_STANDARD: 1, ROLE_ADMIN: 2, ROLE_WAGNERM: 3, OLD_ROLE_WAGNERM: 3}
...
class FiBuMateApp:
    def __init__(self):
        ...
        self.bump_version_once(
            "2026-05-15_close_yearly_history_ids_reports_delegate_template",
            [
                "Abschlusskalender erweitert: Jahresabschluss-Modul für Geschäftsjahre vom 01.10. bis 30.09. ergänzt.",
                "Monats-, Quartals- und Jahresabschluss erweitert: Aufgaben erhalten dauerhaft eindeutige Aufgaben-IDs inklusive Unteraufgaben-Postfix -uX.",
                "Monats-, Quartals- und Jahresabschluss erweitert: Delegierungen können einmalig oder permanent für Folgezeiträume erfolgen; permanente Delegierung läuft über die Aufgaben-ID.",
                "Abschlusskalender erweitert: Aufgaben-Historie nach ID mit Zeitraumdetails und PDF-Berichten ergänzt.",
                "Abschlusskalender erweitert: Abschlussberichte und Aufgaben-ID-Gesamtberichte können als PDF erstellt werden.",
                "Abschlusskalender angepasst: Folgezeiträume können über „Diesen Zeitraum als Vorlage … verwenden“ vollständig anhand der Aufgaben-ID synchronisiert werden.",
            ],
        )
    ...
    def draw_tile_icon_image(self, tile, icon_type, cx, cy):
        mapping = {"calendar": "calendar", "gear": "gear", "info": "info", "lock": "lock", "worksheet": "xls", "modules": "xls"}
    ...
    def ensure_permissions_defaults(self):
        ...
        for key, u in users.items():
            ...
            if key == SUPERUSER_KEY:
                ...
                if u.get("permission") in (ROLE_WAGNERM, OLD_ROLE_WAGNERM):
    ...
    def render_main_menu(self):
        ...
        w, h = self.canvas.winfo_width(), self.canvas.winfo_height()
        tw = max(230, min(310, int(w * 0.155)))
        th = max(125, min(170, int(h * 0.155)))
        gap_x = max(34, int(w * 0.035))
        ...
        def create_tile_group(items, center_y_percent, id_prefix, tile_w=tw, tile_h=th, y_override=None):
            xs = centered_x_positions(len(items), tile_w)
            y = y_override if y_override is not None else y_pct(h, center_y_percent)
        ...
        create_tile_group(app_tiles, 60, "main_app")
        old_footer_center_y = y_pct(h, 17.5)
        footer_tw = int(tw * 0.85)
        footer_th = int(th * 0.85)
        footer_center_y = old_footer_center_y + th / 2 - footer_th / 2
        footer_top = footer_center_y - footer_th / 2
        create_tile_group(footer_tiles, 17.5, "main_footer", tile_w=footer_tw, tile_h=footer_th, y_override=footer_center_y)
    ...
    def render_users_menu(self):
        ...
        users = self.user_data.setdefault("users", {})
        visible_keys = [self.current_user_key] if self.my_role() == ROLE_STANDARD and self.current_user_key in users else sorted(users.keys())
        ...
        for col, header in enumerate(["Benutzer", "Anzeigename", "E-Mail", "Rolle", "Passwort", "Speichern", "Löschen"]):
        ...
        def can_edit_user(target_key):
            ...
            target_role = users.get(target_key, {}).get("permission", ROLE_STANDARD)
            return self.my_role() == ROLE_WAGNERM or self.role_rank(target_role) < self.role_rank(self.my_role())
        def save_user_row(old_key, name_var, email_var):
            if old_key not in users: return
            data = users[old_key]; data["email"] = email_var.get().strip()
            may_rename = self.my_role() in (ROLE_ADMIN, ROLE_WAGNERM) and old_key not in (self.current_user_key, SUPERUSER_KEY) and can_edit_user(old_key)
            if may_rename:
                ...
                data = users.pop(old_key); data["display_name"] = new_name; users[new_key] = data
        ...
        for key in visible_keys:
            ...
            name_var = tk.StringVar(value=user.get("display_name", key)); email_var = tk.StringVar(value=user.get("email", ""))
            name_state = "normal" if self.my_role() in (ROLE_ADMIN, ROLE_WAGNERM) and key not in (self.current_user_key, SUPERUSER_KEY) else "disabled"
            tk.Entry(frame, textvariable=name_var, font=("Segoe UI", 10), width=24, bg="#E8EEF5", fg=TEXT, relief="solid", bd=1, state=name_state).grid(row=row, column=1, sticky="w", padx=(0, 14), pady=4)
            tk.Entry(frame, textvariable=email_var, font=("Segoe UI", 10), width=32, bg="#E8EEF5", fg=TEXT, relief="solid", bd=1).grid(row=row, column=2, sticky="w", padx=(0, 14), pady=4)
            tk.Label(frame, text=user.get("permission", ROLE_STANDARD), bg=BG, fg=TEXT2, font=("Segoe UI", 10)).grid(row=row, column=3, sticky="w", padx=(0, 14), pady=4)
            tk.Label(frame, text="aktiv" if user.get("auth", {}).get("enabled") else "nicht aktiv", bg=BG, fg=TEXT2, font=("Segoe UI", 10)).grid(row=row, column=4, sticky="w", padx=(0, 14), pady=4)
            editable = can_edit_user(key)
            tk.Button(frame, text="Speichern", command=lambda k=key, n=name_var, e=email_var: save_user_row(k, n, e), bg=BLUE if editable else GREY_DISABLED, fg="white", bd=0, padx=10, pady=5, cursor="hand2" if editable else "arrow", state="normal" if editable else "disabled").grid(row=row, column=5, sticky="w", padx=(0, 8), pady=4)
            del_ok = editable and key not in (self.current_user_key, SUPERUSER_KEY) and self.my_role() in (ROLE_ADMIN, ROLE_WAGNERM)
            tk.Button(frame, text="Löschen", command=lambda k=key: delete_user(k), bg=RED if del_ok else GREY_DISABLED, fg="white", bd=0, padx=10, pady=5, cursor="hand2" if del_ok else "arrow", state="normal" if del_ok else "disabled").grid(row=row, column=6, sticky="w", padx=(0, 8), pady=4)
            row += 1
        if self.can_create_users():
            ...
            username_var = tk.StringVar(); email_new_var = tk.StringVar()
            ...
            tk.Label(footer, text="E-Mail", bg=BG, fg=TEXT, font=("Segoe UI", 10, "bold")).grid(row=1, column=2, sticky="e", padx=(0, 4))
            tk.Entry(footer, textvariable=email_new_var, font=("Segoe UI", 10), width=32, bg="#E8EEF5", fg=TEXT, relief="solid", bd=1).grid(row=1, column=3, sticky="w", padx=(0, 14))
            def create_user():
                ...
                users[key] = {"display_name": raw_name, "email": email_new_var.get().strip(), "favorites": [], "auth": {"password_hash": None, "enabled": False}, "permission": ROLE_WAGNERM if key == SUPERUSER_KEY else ROLE_STANDARD}
            ...
            tk.Button(footer, text="Benutzer anlegen", command=create_user, bg=BLUE, fg="white", bd=0, padx=14, pady=8, cursor="hand2").grid(row=1, column=4, sticky="w")
        update_scrollregion()
    ...
    def render_permissions_menu(self):
        ...
        for key in sorted(users.keys()):
            ...
            def save_role(user_key=key, var=role_var):
                ...
                if user_key != SUPERUSER_KEY and new_role == ROLE_WAGNERM: messagebox.showwarning("FiBu Mate", "Die Rolle System-Administrator darf nur der Benutzer Wagnerm tragen."); return
    ...
    def login_user(self, username):
        ...
        users = self.user_data.setdefault("users", {})
        if key not in users: users[key] = {"display_name": username, "favorites": [], "email": "", "auth": {"password_hash": None, "enabled": False}, "permission": ROLE_WAGNERM if key == SUPERUSER_KEY else ROLE_STANDARD}
        else:
            users[key].setdefault("display_name", username); users[key].setdefault("favorites", []); users[key].setdefault("email", ""); users[key].setdefault("auth", {"password_hash": None, "enabled": False})
            if key == SUPERUSER_KEY: users[key]["permission"] = ROLE_WAGNERM
            elif users[key].get("permission") in (ROLE_WAGNERM, OLD_ROLE_WAGNERM): users[key]["permission"] = ROLE_STANDARD
