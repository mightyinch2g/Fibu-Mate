...
def current_period_key():
    return max(fiscal_year_key(), MIN_PERIOD)
...
def ensure_period_window():
    ...
    for offset in range(0, 2):
        p = add_period(current, offset); load_period(p); apply_catalog_to_period(p)
...
class YearlyCloseUI:
    ...
    def normalize_task_uid_value(self, value):
        ...
        if not raw:
            return ""
        allowed_prefixes = ["QMJ", "QM", "QJ", "MJ", CLOSING_SCOPE]
    ...
    def collect_used_task_uids(self):
        used = set()
    ...
    def task_identity_key_for_initial_id(self, task):
        ...
        return ("title", normalize_team_name(task.get("team")), str(task.get("title") or "").strip().casefold())
    ...
    def ensure_task_ids(self):
        ...
        for path in self.all_period_files():
            ...
            for task in data.get("tasks", []):
                uid = self.normalize_task_uid_value(task.get("task_uid", ""))
                key = self.task_identity_key_for_initial_id(task)
                if uid:
                    known.setdefault(key, uid)
                    used.add(uid)
        next_num = 1
        def next_uid():
            nonlocal next_num
        ...
        for path in self.all_period_files():
            ...
            for task in data.get("tasks", []):
                ...
                uid = self.normalize_task_uid_value(task.get("task_uid", ""))
                if not uid:
                    uid = known.setdefault(key, next_uid())
                    task["task_uid"] = uid
                    changed = True
                for i, sub in enumerate(task.get("subtasks", []), start=1):
                    sub_uid = f"{uid}-u{i}"
                    if sub.get("task_uid") != sub_uid:
                        sub["task_uid"] = sub_uid
                        changed = True
            if changed:
        ...
        for task in self.data.get("tasks", []):
            if not task.get("task_uid"):
                task["task_uid"] = known.setdefault(self.task_identity_key_for_initial_id(task), next_uid())
                changed_current = True
            for i, sub in enumerate(task.get("subtasks", []), start=1):
                sub["task_uid"] = f"{task.get('task_uid')}-u{i}"
        if changed_current:
            save_period(self.period, self.data)
    ...
    def ask_delegate_scope(self, item, parent_task=None):
        # Unteraufgaben dürfen nicht permanent delegiert werden.
        ...
        tk.Button(frame, text="Nur dieser Zeitraum", command=lambda: choose("current"), bg=COLORS["blue"], fg="white", bd=0, padx=12, pady=7).pack(fill="x", pady=(0,6))
        tk.Button(frame, text="Permanent für Folgezeiträume", command=lambda: choose("permanent"), bg=COLORS["orange"], fg="white", bd=0, padx=12, pady=7).pack(fill="x", pady=(0,6))
        tk.Button(frame, text="Abbrechen", command=lambda: choose(None), bg=COLORS["header"], fg=COLORS["text"], bd=0, padx=12, pady=7).pack(fill="x")
        win.wait_window()
        return result["scope"]
    ...
    def create_simple_pdf(self, title, rows):
        path = filedialog.asksaveasfilename(title="PDF speichern", defaultextension=".pdf", filetypes=[("PDF-Dateien", "*.pdf")], initialfile=title.replace(" ", "_") + ".pdf")
        if not path: return
        try:
            ...
            if rows:
                ...
                tbl.setStyle(TableStyle([("BACKGROUND", (0,0), (-1,0), colors.HexColor("#D3DEE9")), ("GRID", (0,0), (-1,-1), 0.4, colors.grey), ("FONTNAME", (0,0), (-1,0), "Helvetica-Bold"), ("VALIGN", (0,0), (-1,-1), "TOP")]))
                story.append(tbl)
            doc.build(story)
    ...
    def create_close_report(self):
        rows = [["Aufgaben-ID", "Aufgabe", "Team", "Zuständig", "Status", "Fällig"]]
        for task in self.tasks():
            rows.append([self.normalize_task_uid_value(task.get("task_uid")), task.get("title", ""), task.get("team", ""), task.get("owner", ""), task.get("status", ""), format_date_de(task.get("due_date", ""))])
        self.create_simple_pdf(f"Abschlussbericht {period_label(self.period)}", rows)
    ...
    def show_delegate_popup(self, item, parent_task=None):
        ...
        for key, display in choices:
            ...
            labels.append(label); label_to_choice[label] = (key, display)
            if key == current_key: current_label = label
        if not labels:
            messagebox.showwarning("FiBu Mate", "Keine Benutzer für die Delegierung vorhanden."); return
        if current_label is None: current_label = labels[0]
        win = tk.Toplevel(self.root); win.title("Zuständigkeit delegieren"); win.configure(bg=COLORS["bg"]); win.transient(self.root); win.grab_set(); win.geometry("460x190")
        ...
        selected = tk.StringVar(value=current_label); menu = tk.OptionMenu(win, selected, *labels); menu.config(bg=COLORS["white"], fg=COLORS["text"], bd=1, highlightthickness=0); menu.pack(fill="x", padx=16, pady=(0, 14))
        def apply_delegate():
            ...
            for target in targets:
                target["owner_user_key"] = user_key; target["owner"] = owner_name
            self.save()
            changed = 0
            ...
            if self.selected_team: self.render_team_detail(self.selected_team)
            win.destroy()
            if scope == "permanent": messagebox.showinfo("Delegierung", f"Permanente Delegierung übertragen. Folgezeiträume aktualisiert: {changed}")
        footer = tk.Frame(win, bg=COLORS["bg"]); footer.pack(fill="x", padx=16, pady=(0, 14))
    ...
    def change_period(self, period):
        if period < MIN_PERIOD:
            messagebox.showinfo("Jahresabschluss", "Der Jahresabschluss kann nicht weiter zurück als April 2026 geöffnet werden.")
            return
    ...
    def save_cutoff_from_entry(self, entry_var):
        ...
        if not d:
            messagebox.showwarning("Jahresabschluss", "Bitte einen gültigen Abschluss-Stichtag im Format TT.MM.JJJJ eingeben.")
            return
    ...
    def render_dashboard(self):
        ...
        tk.Button(title_row, text="Abschlussbericht erstellen", command=self.create_close_report, bg=COLORS["white"], fg=COLORS["blue"], bd=1, padx=10, pady=4, cursor="hand2").pack(side="left", padx=(12, 0))
    ...
    def toggle_subtasks_visibility(self, task_id):
        # task_id ist aus Kompatibilitätsgründen der Expand-Key.
