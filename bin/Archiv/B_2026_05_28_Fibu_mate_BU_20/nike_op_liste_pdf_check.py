...
class NikeOPListePDFCheckUI:
    ...
    def path_row(self, parent, row, label, variable, command):
        ...
        btn = tk.Button(parent, text="Auswählen", command=command, bg=BLUE, fg="white", bd=0, padx=12, pady=6, cursor="hand2", compound="right" if photo else "none", image=photo if photo else "", anchor="center")
        if photo:
            btn.image = photo
