...
DUE_LABEL_TO_VALUE = {
    ...
    "x. Werktag des Quartals": DUE_WORKDAY_MONTH,
    "x. Werktag des Folgequartals": DUE_WORKDAY_NEXT,
}
...
def quarter_start(period):
    ...
    return date(int(y), start_month, 1)


def quarter_end(period):
    start = quarter_start(period)
    end_month = start.month + 2
    last_day = calendar.monthrange(start.year, end_month)[1]
    return date(start.year, end_month, last_day)


def due_date_for(period, day_next_quarter):
    next_period = add_quarter(period, 1)
    start = quarter_start(next_period)
    day = max(1, min(int(day_next_quarter), calendar.monthrange(start.year, start.month)[1]))
    return f"{start.year:04d}-{start.month:02d}-{day:02d}"
...
def resolve_due_date(task, period):
    ...
    if mode == DUE_WORKDAY_MONTH:
        start = quarter_start(period)
        current = start
        target = int(task.get("due_workday") or 1)
        count = 0
        while True:
            if is_business_day(current):
                count += 1
    ...
    if mode == DUE_WORKDAY_NEXT:
        start = quarter_start(add_quarter(period, 1))
        current = start
        target = int(task.get("due_workday") or 1)
        count = 0
        while True:
...
def due_rule_text(task):
    if task.get("due_mode") == DUE_WORKDAY_MONTH:
        return f"{task.get('due_workday') or 1}. Werktag Quartal"
    if task.get("due_mode") == DUE_WORKDAY_NEXT:
        return f"{task.get('due_workday') or 1}. Werktag Folgequartal"
    return ""
...
def apply_catalog_to_period(period):
    ...
    for entry in catalog.get("tasks", []):
        if not entry.get("recurring", True):
            continue
        start_period = entry.get("start_period", quarter_key())
...
class QuarterlyCloseUI:
    ...
    def render_period_controls(self, parent):
        ...
        tk.Button(row, text="< vorheriges Quartal", command=lambda: self.change_period(add_quarter(self.period, -1)), bg=COLORS["blue"], fg="white", bd=0, padx=12, pady=6).pack(side="left")
        ...
        tk.Button(row, text="nächstes Quartal >", command=lambda: self.change_period(add_quarter(self.period, 1)), bg=COLORS["blue"], fg="white", bd=0, padx=12, pady=6).pack(side="left")
    ...
    def render_edit_tools(self, parent, team=None):
        ...
        tk.Button(row, text="Aufgaben dieses Quartals allen Quartalen zuweisen", command=self.apply_current_tasks_to_all_months, bg=COLORS["orange"], fg="white", bd=0, padx=12, pady=5).pack(side="left", padx=8)
    ...
    def render_dashboard(self):
        ...
        tk.Label(top, text=f"Quartalsabschluss {period_label(self.period)}", bg=COLORS["white"], fg=COLORS["text"], font=("Segoe UI", 22, "bold")).pack(anchor="w", padx=14, pady=(6, 2))
    ...
    def toggle_done(self, task):
        ...
        if real.get("status") == STATUS_DONE:
            ...
            if real.get("deadline_type") == "gesetzlich" and not messagebox.askyesno("Quartalsabschluss", "Gesetzliche Frist: Aufgabe wirklich als erledigt markieren?"):
                return
    ...
    def open_task_dialog(self, team, task=None):
        ...
        def save_dialog():
            ...
            if not title: messagebox.showwarning("Quartalsabschluss", "Bitte einen Aufgabennamen eingeben."); return
            workday_value = None
            if mode != DUE_FIXED:
                try:
                    workday_value = int(due_workday_var.get().strip()); assert workday_value > 0
                except Exception:
                    messagebox.showwarning("Quartalsabschluss", "Bitte eine gültige positive Werktag-Nummer eingeben."); return
            elif due_var.get().strip() and not parse_date(due_var.get().strip()):
                messagebox.showwarning("Quartalsabschluss", "Bitte ein gültiges Fälligkeitsdatum im Format YYYY-MM-DD eingeben."); return
    ...
    def apply_current_tasks_to_all_months(self):
        ...
        self.reload(); messagebox.showinfo("Aufgaben übertragen", "Die Aufgaben dieses Quartals wurden allen vorhandenen Quartalen zugewiesen.")
        self.render_team_detail(self.selected_team) if self.selected_team else self.render_dashboard()
