...
class MonthlyCloseUI:
    ...
    def save_team_members_from_widget(self, team, widget):
        set_team_members_text(self.data, team, widget.get("1.0", "end")); self.save(); self.render_dashboard()
    ...
    def render_task_table(self, team):
        table = tk.Frame(self.frame, bg=COLORS["white"], bd=1, relief="solid"); table.pack(fill="both", expand=True, padx=24, pady=(0, 12))
        headers = ["Status", "Aufgabe", "Zuständig", "Fällig", "Fristart", "Priorität", "Wiederkehrend", "Anlagen", "Aktion"]
        if self.edit_mode and self.can_edit(): headers.append("Bearbeiten")
        for col, h in enumerate(headers): tk.Label(table, text=h, bg=COLORS["header"], fg=COLORS["text"], font=("Segoe UI", 10, "bold"), padx=8, pady=6).grid(row=0, column=col, sticky="nsew")
        row_idx = 1
        for task in self.team_tasks(team): sync_parent_status_from_subtasks(task); row_idx = self.render_task_row(table, row_idx, task, headers)
        for col in range(len(headers)): table.grid_columnconfigure(col, weight=1 if col in (1, 2, 3) else 0)
    ...
    def render_task_row(self, table, row_idx, task, headers):
        ...
        btn = tk.Button(table, text="✓" if task.get("status") == STATUS_DONE else "□", command=lambda t=task: self.toggle_done(t), bg="#BBF7D0" if task.get("status") == STATUS_DONE else bg, fg=COLORS["dark_green"] if task.get("status") == STATUS_DONE else COLORS["text"], bd=0, font=("Segoe UI", 13, "bold")); btn.grid(row=row_idx, column=0, sticky="nsew", padx=1, pady=1)
        if task.get("subtasks") and not can_finish:
        ...
        for col, val in enumerate(values, 1): tk.Label(table, text=val, bg=bg, fg=COLORS["text"], font=("Segoe UI", 10), padx=8, pady=6, anchor="w", justify="left").grid(row=row_idx, column=col, sticky="nsew", padx=1, pady=1)
        ...
        status_var = tk.StringVar(value=task.get("status", STATUS_OPEN)); menu = tk.OptionMenu(table, status_var, *STATUSES, command=lambda value, t=task: self.set_status(t, value)); menu.config(bg=bg, fg=COLORS["text"], bd=0, highlightthickness=0); menu.grid(row=row_idx, column=8, sticky="nsew", padx=1, pady=1)
        if self.edit_mode and self.can_edit():
    ...
    def toggle_done(self, task):
        ...
        if not real: return
        if real.get("subtasks") and not all_subtasks_done(real):
            self.show_tooltip(self.root, "Bitte erst alle Unteraufgaben erledigen."); self.root.after(1600, self.hide_tooltip); return
    ...
    def set_status(self, task, status):
        ...
        if status == STATUS_DONE and real.get("subtasks") and not all_subtasks_done(real):
            messagebox.showinfo("Monatsabschluss", "Bitte erst alle Unteraufgaben erledigen."); self.render_team_detail(real["team"]); return
    ...
    def open_task_dialog(self, team, task=None):
        ...
        data = dict(task) if task else {"title": "", "owner": team, "owner_user_key": "", "deadline_type": "keine", "priority": "normal", "due_mode": DUE_CUTOFF, "due_day": 1, "due_workday": 1, "due_fixed_date": "", "recurring": False, "subtasks": []}
        ...
        title_var = tk.StringVar(value=data.get("title", "")); deadline_var = tk.StringVar(value=data.get("deadline_type", "keine") if data.get("deadline_type") in DEADLINE_TYPES else "keine"); priority_var = tk.StringVar(value=data.get("priority", "normal")); recurring_var = tk.BooleanVar(value=bool(data.get("recurring")))
