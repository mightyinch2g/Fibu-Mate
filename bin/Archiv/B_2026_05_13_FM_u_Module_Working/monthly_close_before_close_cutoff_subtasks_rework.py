...
def default_due_date(period):
    ...
    return date(y, m, calendar.monthrange(y, m)[1]).strftime("%Y-%m-%d")
...
def resolve_due_date(task, period):
    ...
    return next_business_day(due).strftime("%Y-%m-%d") if due else default_due_date(period)
...
def ensure_period_window():
    ensure_storage()
    cleanup_old_periods()
    current = max(month_key(), MIN_PERIOD)
...
class MonthlyCloseUI:
    ...
    def open_task_dialog(self, team, task=None):
        ...
        data = task or {"title": "", "owner": team, "owner_user_key": "", "due_date": default_due_date(self.period), "deadline_type": "keine", "priority": "normal", "due_mode": DUE_FIXED, "due_workday": None, "recurring": False}
        form = tk.Frame(win, bg=COLORS["bg"]); form.pack(fill="both", expand=True, padx=18, pady=18)
        title_var = tk.StringVar(value=data.get("title", "")); due_var = tk.StringVar(value=format_date_de(data.get("due_date", default_due_date(self.period))))
        ...
        due_date_label = tk.Label(form, text="Konkretes Datum (TT.MM.JJJJ)", bg=COLORS["bg"], fg=COLORS["text"], font=("Segoe UI", 10, "bold"))
        ...
        def update_calculated(*_):
            ...
            calculated_var.set("Berechnetes Fälligkeitsdatum: " + (format_date_de(resolve_due_date(preview, self.period)) or "-"))
        ...
        def save_dialog():
            ...
            if mode != DUE_FIXED:
                ...
                messagebox.showwarning("Monatsabschluss", "Bitte ein gültiges Fälligkeitsdatum im Format TT.MM.JJJJ eingeben."); return
