...
class QuarterlyCloseUI:
    ...
    def set_status(self, task, status):
        ...
        if status == STATUS_DONE and real.get("subtasks") and not all_subtasks_done(real):
            messagebox.showinfo("Quartalsabschluss", "Bitte erst alle Unteraufgaben erledigen."); self.render_team_detail(real["team"]); return
        if status == STATUS_DONE and real.get("deadline_type") == "gesetzlich" and not messagebox.askyesno("Quartalsabschluss", "Gesetzliche Frist: Aufgabe wirklich als erledigt markieren?"):
