...
class FiBuMateApp:
    ...
    def draw_suggestion_control(self):
        if self.current_page == "launch":
            return
        size = 30
        btn = tk.Canvas(self.root, width=size, height=size, bg=HEADER, highlightthickness=0, bd=0, cursor="hand2")
        btn.create_rectangle(1, 1, size - 1, size - 1, fill=HEADER, outline=LINE)
        btn.create_oval(7, 5, 23, 21, fill="#FFF176", outline="#FACC15", width=2)
        btn.create_oval(4, 2, 26, 24, outline="#FFE45C", width=2)
        btn.create_rectangle(11, 20, 19, 25, fill="#F59E0B", outline="#B45309")
        btn.create_line(12, 26, 18, 26, fill="#B45309", width=2)
        btn.create_line(15, 1, 15, 4, fill="#FFF176", width=2)
        btn.create_line(3, 12, 6, 12, fill="#FFF176", width=2)
        btn.create_line(24, 12, 27, 12, fill="#FFF176", width=2)
        ...
        self.canvas.create_window(self.canvas.winfo_width() - 132, 7, window=btn, anchor="ne")
    ...
    def render_users_menu(self):
        if not self.can_view_user_management():
            self.render_menu_text("Keine Berechtigung für die Benutzerverwaltung.")
            return
        ...
        users = self.user_data.setdefault("users", {})
        headers = ["Benutzer", "Anzeigename", "E-Mail", "Rolle", "Passwort", "Speichern", "Löschen"]
        ...
        def save_user_row(old_key, name_var, email_var):
            new_name = " ".join(name_var.get().strip().split())
            new_key = normalize_username(new_name)
            if not new_key:
                messagebox.showwarning("FiBu Mate", "Bitte einen Benutzernamen eingeben.")
                return
            if old_key == SUPERUSER_KEY or new_key == SUPERUSER_KEY:
                messagebox.showwarning("FiBu Mate", "Der Benutzer Wagnerm kann nicht umbenannt werden.")
                return
            if new_key != old_key and new_key in users:
                messagebox.showwarning("FiBu Mate", "Dieser Benutzername existiert bereits.")
                return
            if not can_edit_user(old_key):
                messagebox.showwarning("FiBu Mate", "Keine Berechtigung für diesen Benutzer.")
                return
            data = users.pop(old_key)
            data["display_name"] = new_name
            data["email"] = email_var.get().strip()
            users[new_key] = data
            if self.current_user_key == old_key:
                self.current_user_key = new_key
                self.current_user_display = new_name
            self.save_user_data()
        ...
        def delete_user(user_key):
            if user_key == SUPERUSER_KEY:
                messagebox.showwarning("FiBu Mate", "Der Benutzer Wagnerm kann nicht gelöscht werden.")
                return
            if user_key == self.current_user_key:
                messagebox.showwarning("FiBu Mate", "Der aktuell angemeldete Benutzer kann nicht gelöscht werden.")
                return
            if not can_edit_user(user_key):
                messagebox.showwarning("FiBu Mate", "Keine Berechtigung für diesen Benutzer.")
                return
            display = users.get(user_key, {}).get("display_name", user_key)
            if not messagebox.askyesno("Benutzer löschen", f"Benutzer wirklich löschen?\n\n{display}"):
                return
            users.pop(user_key, None)
            self.save_user_data()
            messagebox.showinfo("FiBu Mate", "Benutzer wurde gelöscht.")
            self.render_page()
        ...
        for key in sorted(users.keys()):
            ...
            name_var = tk.StringVar(value=user.get("display_name", key))
            email_var = tk.StringVar(value=user.get("email", ""))
            tk.Entry(frame, textvariable=name_var, font=("Segoe UI", 10), width=24, bg="#E8EEF5", fg=TEXT, relief="solid", bd=1).grid(row=row, column=1, sticky="w", padx=(0, 14), pady=4)
            ...
            tk.Button(frame, text="Löschen", command=lambda k=key: delete_user(k), bg=RED if editable else GREY_DISABLED, fg="white", bd=0, padx=10, pady=5, cursor="hand2" if editable else "arrow", state="normal" if editable else "disabled").grid(row=row, column=6, sticky="w", padx=(0, 8), pady=4)
            row += 1

        if self.can_create_users():
            tk.Frame(frame, bg=LINE, height=1).grid(row=row, column=0, columnspan=8, sticky="ew", pady=(18, 14))
            row += 1
            tk.Label(frame, text="Neuen Benutzer anlegen", font=("Segoe UI", 12, "bold"), bg=BG, fg=TEXT).grid(row=row, column=0, columnspan=8, sticky="w", pady=(0, 8))
            row += 1
            username_var = tk.StringVar()
            email_new_var = tk.StringVar()
            tk.Label(frame, text="Benutzername", bg=BG, fg=TEXT, font=("Segoe UI", 10, "bold")).grid(row=row, column=0, sticky="w")
            tk.Entry(frame, textvariable=username_var, font=("Segoe UI", 10), width=28, bg="#E8EEF5", fg=TEXT, relief="solid", bd=1).grid(row=row, column=1, sticky="w", padx=(0, 14))
            tk.Label(frame, text="E-Mail", bg=BG, fg=TEXT, font=("Segoe UI", 10, "bold")).grid(row=row, column=2, sticky="w")
            tk.Entry(frame, textvariable=email_new_var, font=("Segoe UI", 10), width=32, bg="#E8EEF5", fg=TEXT, relief="solid", bd=1).grid(row=row, column=3, sticky="w", padx=(0, 14))
            def create_user():
                raw_name = " ".join(username_var.get().strip().split())
                key = normalize_username(raw_name)
                if not key:
                    messagebox.showwarning("FiBu Mate", "Bitte einen Benutzernamen eingeben.")
                    return
                if key in users:
                    messagebox.showwarning("FiBu Mate", "Dieser Benutzer existiert bereits.")
                    return
                ...
                self.ensure_permissions_defaults()
                self.save_user_data()
                messagebox.showinfo("FiBu Mate", f"Benutzer wurde angelegt:\n\n{raw_name}")
                self.render_page()
            tk.Button(frame, text="Benutzer anlegen", command=create_user, bg=BLUE, fg="white", bd=0, padx=14, pady=8, cursor="hand2").grid(row=row, column=4, sticky="w")
        self.canvas.create_window(self.canvas.winfo_width() / 2, y_pct(self.canvas.winfo_height(), 48), window=frame, anchor="center")
