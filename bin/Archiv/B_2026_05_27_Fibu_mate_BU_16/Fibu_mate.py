...
class FiBuMateApp:
    ...
    def render_users_menu(self):
        ...
        for col, header in enumerate(["Benutzer", "Anzeigename/Nachname", "Vorname", "E-Mail", "Rolle", "Passwort", "Speichern", "Löschen"]):
        ...
        for key in visible_keys:
            ...
            tk.Button(frame, text="Speichern", command=lambda k=key, n=name_var, e=email_var, f=first_name_var: save_user_row(k, n, e, f), bg=BLUE if editable else GREY_DISABLED, fg="white", bd=0, padx=10, pady=5, cursor="hand2" if editable else "arrow", state="normal" if editable else "disabled").grid(row=row, column=7, sticky="w", padx=(0, 8), pady=4)
            del_ok = editable and key not in (self.current_user_key, SUPERUSER_KEY) and self.my_role() in (ROLE_E3, ROLE_E4)
            tk.Button(frame, text="Löschen", command=lambda k=key: delete_user(k), bg=RED if del_ok else GREY_DISABLED, fg="white", bd=0, padx=10, pady=5, cursor="hand2" if del_ok else "arrow", state="normal" if del_ok else "disabled").grid(row=row, column=7, sticky="w", padx=(0, 8), pady=4)
            row += 1
    ...
    def login_user(self, username):
        ...
        users = self.user_data.setdefault("users", {})
        if key not in users: users[key] = {"display_name": username, "favorites": [], "email": "", "auth": {"password_hash": None, "enabled": False}, "permission": ROLE_E4 if key == SUPERUSER_KEY else ROLE_E1}
        else:
            ...
            if key == SUPERUSER_KEY: users[key]["permission"] = ROLE_E4
            elif users[key].get("permission") in (ROLE_E4, OLD_ROLE_E4): users[key]["permission"] = ROLE_E1
            else: users[key].setdefault("permission", ROLE_E1)
