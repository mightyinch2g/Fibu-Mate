"""FiBu Mate – AFI-Upload, deterministisches Lieferanten-Regelwerk 1.1.0.
...
class RuleEngine:
    ...
    def _enbw(self, source_rows, path):
        ...
        for source in source_rows:
            ...
            if energy_value:
                key = (company, costcenter, account, resolved["name"] or plate or "ohne Zuordnung")
        ...
        output = []
        for (company, costcenter, account, name), value in sorted(energy.items()):
            output.append(_row(f"EnBW Stromtanken {name}", value, "V2", account, costcenter))
        ...
        for company, value in sorted(base_fees.items()):
            ...
            output.append(_row(f"EnBW Grundgebühr {company}", value, "V2", account, costcenter))
            if not costcenter:
    ...
    def _kazenmaier(self, text, path):
        ...
        for match in pattern.finditer(text):
            ...
            output.append(_row(f"Kazenmaier Bike Leasing {resolved['name'] or name}", _money(total), "V2", account, resolved["costcenter"], order_id))
    ...
    def _dkv(self, text, path):
        ...
        for block in blocks:
            ...
            output.append(_row(f"DKV Tanken {plate} {resolved['name']}", net, "V2", account, resolved["costcenter"]))
            if not resolved["costcenter"]:
    ...
    def _vodafone(self, text, path):
        ...
        for (costcenter, company, gl_account), value in grouped.items():
            output.append(_row(f"Vodafone Mobilfunk {company} ({len(labels[(costcenter, company, gl_account)])} Anschlüsse)", value, "V2", gl_account, costcenter))
        if not output:
...
class AFIUI:
    def __init__(self, app):
        ...
        self.status_var = tk.StringVar(value="Bereit – Standardfälle ohne ONNX/LLM."); self.summary = tk.StringVar(value="Noch kein Ergebnis."); self.result = None

    def render(self):
        try:
            self.canvas.delete("all"); self.app.draw_background(); self.app.draw_header(MODULE_TITLE); self.app.draw_path_bar()
        except Exception:
            pass
        ...
        tk.Label(files, text=f"Zentral für alle Anwender gespeichert: {_shared_settings_file(False)}", bg=self.bg, fg="#44536A", anchor="w").grid(row=3, column=0, columnspan=4, sticky="ew", pady=(5, 0))
        actions = tk.Frame(frame, bg=self.bg); actions.grid(row=1, column=0, sticky="ew", padx=8)
        self.run = tk.Button(actions, text="2. Lokal verarbeiten", command=self.start, bg="#0F6CBD", fg="white", padx=12, pady=6); self.run.pack(side="left")
        tk.Button(actions, text="Regelwerk-Status", command=self.check, padx=10, pady=6).pack(side="left", padx=8)
        ...
        for column in UPLOAD_COLUMNS:
            self.tree.heading(column, text=column); self.tree.column(column, width=240 if column == "TEXT" else 95)
        self.tree.grid(row=0, column=0, sticky="nsew"); self.tree.bind("<Double-1>", self.edit)
        ybar = ttk.Scrollbar(holder, orient="vertical", command=self.tree.yview); xbar = ttk.Scrollbar(holder, orient="horizontal", command=self.tree.xview)
        ybar.grid(row=0, column=1, sticky="ns"); xbar.grid(row=1, column=0, sticky="ew"); self.tree.configure(yscrollcommand=ybar.set, xscrollcommand=xbar.set)
        ...
        self.save = tk.Button(footer, text="Prüf-JSON speichern", command=self.export_json, state="disabled"); self.save.pack(side="left", padx=8)
    ...
    def pick_db(self, variable, title):
        ...
        if path:
            variable.set(path); _save_settings(self.costcenter.get(), self.account.get())

    def clear_db(self, variable):
        variable.set(""); _save_settings(self.costcenter.get(), self.account.get())
    ...
    def start(self):
        try:
            invoice, costcenter, account = self._inputs(); _save_settings(costcenter, account)
        except Exception as exc:
            messagebox.showwarning(MODULE_TITLE, str(exc)); return
        ...
        # Kein nativer KI-Aufruf. Ein normaler Python-Thread kann FiBu Mate nicht durch ONNX beenden.
    ...
    def worker(self, invoice, costcenter, account):
        try:
            master = MasterData(costcenter, account); engine = RuleEngine(master)
    ...
    def done(self, result):
        ...
        self.run.config(state="normal"); self.save.config(state="normal"); self.export.config(state="normal" if validation["export_allowed"] else "disabled"); self.status_var.set("Fertig – deterministisch verarbeitet.")
    ...
    def fail(self, message, details):
        ...
        directory = Path(os.environ.get("LOCALAPPDATA", str(Path.home()))) / "FiBuMate" / "FoundryLocal" / "logs"; directory.mkdir(parents=True, exist_ok=True)
        path = directory / f"afi_rules_{datetime.now():%Y%m%d_%H%M%S}.log"; path.write_text(details, encoding="utf-8")
        messagebox.showerror(MODULE_TITLE, f"{message}\n\nProtokoll: {path}")

    def check(self):
        messagebox.showinfo(MODULE_TITLE, f"Regelwerk bereit: {RULESET_VERSION}\n\nFreigegeben: EnBW Charging CSV, DKV, Vodafone Mobilfunk, Kazenmaier Bike Leasing.\n\nStandardfälle laufen ohne ONNX/LLM und können FiBu Mate daher nicht durch einen nativen Modellfehler beenden.")
    ...
    def edit(self, event):
        ...
        if new is not None:
            self.tree.set(item, column, new); self.result["upload_rows"][self.tree.index(item)][column] = new
