...
class FoundryLocalProvider:
    ...
    def process(self,invoice,refs,progress):
        ...
        try:
            ...
            client=model.get_chat_client();client.settings.temperature=float(self.cfg.get("temperature",0));client.settings.max_tokens=int(self.cfg["max_output_tokens"]);client.settings.random_seed=int(self.cfg.get("random_seed",42));client.settings.response_format={"type":"json_schema","json_schema":schema}
